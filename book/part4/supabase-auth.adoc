...
쿠키(cookie)는 서버가 브라우저에 맡겨 두는 작은 기록이고, 그중 HttpOnly라는 표시가 붙은 쿠키는 그 페이지의 자바스크립트가 들여다볼 수 없다.
표시의 뜻과 이유는 "HttpOnly 쿠키에 담는 로그인 세션" 절에서 코드와 함께 다시 본다.
...
로그인을 따라 만드는 데는 이 장만으로 충분하다.

무료 플랜의 한도와 일시정지·백업 같은 운영 문제는 {ch-free-tier}에서 Vercel과 함께 다룬다.
...
앱을 세상에 공개하는 순간 이 전제는 더 이상 성립하지 않는다.
요청을 보내는 사람이 여럿이고, 서로가 서로의 덱과 학습 기록을 봐서는 안 되며, "나는 이 사용자다"라는 주장을 말 그대로 믿어 줄 수 없다.
그 주장을 위조할 수 없는 방식으로 확인하는 일이 인증이다.
확인 뒤에도 토큰 수명, 탈취, 위조 서명 같은 공격면이 줄줄이 따라온다.
이 장의 나머지는 운영에서 저 몇 줄짜리 미들웨어를 무엇으로 대신해야 하는지 다룬다.
...
기능 요구사항을 소화하려면 사용자별 덱과 카드, 학습 기록을 저장할 관계형 데이터베이스가 있어야 하고, "사용자별"이라는 말이 성립하려면 로그인이 필요하다.
...
둘째, 소셜 로그인 연동이다.
Google과 GitHub 각각에 OAuth 앱을 등록하고, 인가 코드(authorization code)를 토큰으로 교환하는 절차를 구현하고, 프로바이더(provider)마다 다른 응답 형식과 오류 케이스를 처리해야 한다.
예를 들어 GitHub는 사용자가 이메일을 비공개로 두면 프로필 API의 `email` 필드를 `null`로 돌려주므로footnote:[GitHub REST API 문서 "Users"의 응답 설명. https://docs.github.com/en/rest/users/users[Users - GitHub Docs]], 이메일을 따로 조회하는 분기가 GitHub에만 필요하다.
이 장에서 계속 만날 인가 코드·인가 화면·인가 주소의 "인가"는 사용자가 프로바이더 화면에서 우리 앱의 로그인 요청을 허가하는 OAuth 절차를 가리킨다.
{ch-database-basics}에서 본 인가(그 사람이 이 일을 해도 되는지 앱 안에서 확인하는 권한 검사)와는 다른 뜻이다.

.[용어 풀이] 프로바이더(Provider)
[NOTE]
====
OAuth에서 사용자의 신원을 대신 확인해 주는 쪽, 곧 Google이나 GitHub처럼 이미 그 사람의 계정이 있는 서비스를 프로바이더라 부른다.
우리 앱은 비밀번호를 받지 않고 "이 사람이 맞다"는 프로바이더의 확인 결과만 건네받는다.
신원 확인을 여권 발급 기관에 맡기고 우리는 여권을 확인만 하는 것과 같다.
프로바이더마다 응답 형식과 오류 처리가 다른데(위의 GitHub 비공개 이메일이 한 예다), Flashcard에서는 그 차이를 Supabase가 대신 흡수한다.
====

셋째, 토큰 수명 관리다.
액세스 토큰(access token)의 발급·만료·갱신, 리프레시 토큰(refresh token)의 회전과 폐기, 로그아웃 시 무효화까지, 토큰이 언제 생기고 언제 바뀌고 언제 못 쓰게 되는지를 정하는 규칙 전체를 직접 운영해야 한다.
...
여기까지가 비밀번호를 포함한 일반론이다.
비밀번호를 받지 않는 이 앱에 한정해서 "Google과 GitHub 로그인은 그대로 두고 Supabase Auth만 걷어내면 무엇이 남는가"를 따진 목록은 {ch-alternatives}에 있다.
그 목록에서 가장 큰 변화는 토큰의 발급자가 우리로 바뀐다는 것인데, 그것이 무엇을 뜻하는지는 이 장의 "무상태" 절을 읽고 나면 분명해진다.
...
| Firebase | 인증 + NoSQL DB + 호스팅 | Firestore는 관계형 스키마에 부적합, 벤더 종속 | 모바일 중심의 실시간 동기화 앱
...
그러나 Firestore는 NoSQL 문서 저장소라서 {ch-database-design}에서 설계한 관계형 스키마(외래 키 제약, JOIN, 집계 쿼리)를 그대로 옮길 수 없다.
더 큰 문제는 벤더 종속(vendor lock-in)이다.
Firestore 전용 API로 작성한 데이터 계층은 다른 곳으로 이식할 수 없지만, Supabase의 DB는 표준 PostgreSQL이라 `pg_dump` 한 번으로 어디로든 옮길 수 있다.
...
자체 호스팅은 VPS(Virtual Private Server), 곧 매달 요금을 내고 빌려 쓰는 가상 서버에 PostgreSQL과 인증 서버를 직접 올리는 방식이다.
Supabase 자체가 오픈소스라서 Supabase를 통째로 직접 올릴 수도 있다.
...
Supabase의 공식 퀵스타트는 supabase-js(와 서버 렌더링용 `@supabase/ssr`)를 프런트엔드 코드에서 불러 로그인하고 데이터를 읽는 흐름을 안내한다footnote:[Next.js 퀵스타트가 `@supabase/supabase-js`와 `@supabase/ssr`이 설치된 템플릿에서 출발한다. https://supabase.com/docs/guides/getting-started/quickstarts/nextjs[Use Supabase with Next.js]].
그러나 Flashcard의 화면은 {ch-html-template}과 {ch-htmx}에서 본 대로 서버에서 렌더링되고, 곧 확인하겠지만 로그인 흐름에도 브라우저 자바스크립트가 필요 없다.
대신 Go 서버가 Supabase Auth의 REST API(GoTrue라는 이름의 오픈소스 인증 서버다)와 직접 대화하는데, 그 클라이언트 전체가 `internal/web/gotrue.go`의 130줄 남짓이다.

이렇게 절제한 덕에 벤더 종속도 줄어든다.
...
플랫폼이 주는 기능을 전부 써야 본전이라는 생각은 오히려 이식성을 떨어뜨린다.
필요한 것만 골라 쓰는 실용주의가 이 선택의 핵심이다.

== 서버 사이드 인증: 브라우저에는 토큰이 없다

이제 로그인 버튼을 누른 순간부터 Go 핸들러가 사용자를 식별하기까지의 전체 흐름을 따라가 보자.
일상어로 먼저 요약하면, 브라우저가 세 곳을 차례로 다녀와 쿠키 하나를 받아 오는 여정이다.
큰 그림은 그림 25에 있다.
세로줄 네 개는 각각 브라우저, 우리 Go 서버, Supabase의 인증 서버(GoTrue), Google·GitHub이다.
가로 화살표는 요청이 가는 방향이고, ①부터 ⑥까지의 번호가 시간 순서다.

++++
<div class="fc-shots">

![브라우저, Go 서버, GoTrue, Google/GitHub 네 세로줄 사이를 여섯 개의 번호 붙은 화살표가 오가는 로그인 시퀀스](/screenshots/oauth-sequence.png)

</div>

<p class="fc-caption">그림 25 OAuth 로그인의 여섯 단계. 브라우저는 Go 서버와 프로바이더를 다녀와 코드 하나를 들고 돌아오고, 그 코드를 토큰으로 바꾸는 ⑤는 서버와 GoTrue 사이에서만 일어난다. 브라우저가 받는 것은 리다이렉트와 HttpOnly 쿠키뿐이다.</p>
++++

눈여겨볼 것은 두 가지다.
첫째, 프로바이더(Google/GitHub)와 직접 마주 보는 쪽은 GoTrue다.
동의 화면 왕복과 프로바이더별 응답 차이는 모두 Supabase가 흡수하고, 우리 서버는 GoTrue와만 대화한다.
...
링크를 따라가기만 하면 여정이 시작되고, 나머지 일은 전부 서버가 한다.
`?next=`는 로그인이 끝난 뒤 돌아갈 화면을 기억해 두는 파라미터인데, 그 안전 처리(`safeNext`)는 "만료와 리프레시" 절에서 다시 다룬다.
...
핸들러는 다음 "PKCE" 절에서 설명할 검증용 무작위 값(verifier)과 돌아갈 경로를 수명 5분짜리 쿠키에 적어 두고, 브라우저를 GoTrue의 인가 주소로 리다이렉트한다.
...
GoTrue로 떠나보낼 주소 뒤에 물음표를 붙여 값 네 개를 매다는 코드다.
어느 프로바이더로 갈지, 끝나면 어디로 돌아올지(`redirect_to`), 그리고 "PKCE" 절에서 볼 지문 값과 그 지문을 뜬 방식이 그 넷이다.

[source,go]
----
include::../../flashcard-advanced/internal/web/gotrue.go[tag=authorize-url]
----

이 주소로 이동한 브라우저는 GoTrue를 거쳐 Google이나 GitHub의 동의 화면으로 넘어간다(③).
방문자가 동의하면 프로바이더는 그 브라우저를 GoTrue로 돌려보내고, GoTrue는 다시 같은 브라우저를 `redirect_to`에 적힌 우리 앱의 `/auth/callback`으로 돌려보낸다.
이때 URL에 `?code=`라는 인가 코드가 붙어 온다(④).
줄곧 움직이는 것은 브라우저 하나이고, 세 곳은 저마다 "다음은 저기로 가라"고 일러 줄 뿐이다.
...
func (w *Web) oauthCallback(c *gin.Context) {
	...
	// ... (GoTrue가 ?error=로 알린 자기 쪽 실패는 로그에 남기고 로그인 화면으로)
	code := c.Query("code")
	...
	// ... (verifier 쿠키가 만료됐으면 처음부터 다시 시도하라고 안내)
...
실패는 원인별로 따로 처리한다.
GoTrue가 `?error=`로 실패를 알린 경우, 인가 코드가 없는 경우, verifier 쿠키가 만료된 경우, 교환 호출이 실패한 경우다.
어느 분기든 오류 메시지를 플래시 쿠키에 담아 방문자를 로그인 화면으로 되돌리고, 원인은 `log.Printf`로 서버 로그에 남긴다.
방문자에게 보일 문구와 운영자가 볼 기록은 다른 물건이기 때문이다.
...
코드 가로채기를 막는 PKCE가 앞 절의 관문이었다면, 교환으로 얻은 토큰을 어디에 보관하느냐가 이 아키텍처의 또 하나의 승부처다.
...
Lax는 다른 사이트에서 출발한 POST 요청에 쿠키를 싣지 않으므로, Flashcard처럼 데이터를 바꾸는 모든 요청이 POST인 앱에서는 이 한 줄이 CSRF 방어의 핵심이 된다.
예를 들어 악성 사이트에 숨겨 둔 폼이 우리 앱의 `/decks/:slug/delete`로 POST를 보내도, 쿠키가 빠진 채 도착하므로 서버가 보기에는 익명 방문자의 요청일 뿐이다.
반면 다른 사이트의 링크를 눌러 들어오는 평범한 이동(GET)에는 쿠키가 실려, 방문자는 로그인된 상태 그대로 화면을 본다.
...
악성 스크립트를 우리 페이지 안에 심는 XSS({ch-html-template})와는 다른 공격이다.
공격자는 우리 서버의 응답을 읽지 못하고 요청만 대신 보내게 만들 뿐이므로, 방어도 "남의 사이트에서 출발한 요청에는 쿠키를 붙이지 않는다"는 쪽이다.
====

토큰을 실제로 쿠키에 넣는 함수에서는 수명 계산을 눈여겨볼 만하다.
...
리프레시 쿠키의 수명은 `refreshMaxAge` 상수로 정한 30일이다.
Supabase의 리프레시 토큰 자체는 기본 설정에서 만료되지 않고 한 번 쓰면 새 토큰으로 교체될 뿐이며, 세션에 상한을 두는 설정은 Pro 플랜부터 쓸 수 있다footnote:[Supabase 문서 "User sessions"의 리프레시 토큰과 세션 수명 설명. https://supabase.com/docs/guides/auth/sessions[User sessions]].
그래서 30일은 Supabase가 정한 값이 아니라, 이 앱이 브라우저에 로그인 상태를 얼마나 오래 남겨 둘지 정한 값이다.

여기서 대안과 정직하게 비교하면, 차이는 안전의 기본값에 있다.
...
다만 쿠키 방식은 아무 추가 장치 없이 기본 속성 세 개만으로 위의 방어선이 서고, localStorage 방식은 안전해지기 위해 설계를 더 얹어야 한다.
...
모든 페이지 요청은 이 미들웨어를 지나면서 방문자의 신원을 확인받는다.
...
경로가 셋이다.
첫째, 액세스 쿠키의 토큰이 유효하면 그대로 통과시킨다(가장 흔한 경로이고, 네트워크 왕복이 없다).
둘째, 만료됐거나 없으면 리프레시 쿠키로 GoTrue에 갱신을 한 번 요청해 새 토큰을 쿠키에 다시 심는다.
셋째, 그마저 실패하면 망가진 쿠키를 지우고 익명 방문자로 통과시킨다.
...
`safeNext`는 `/`로 시작하는 같은 앱 내부 경로만 허용한다.
다만 `//evil.com`처럼 슬래시 두 개로 시작하는 값은 걸러 낸다.
브라우저는 이것을 앱 안의 경로가 아니라 "지금과 같은 https로 evil.com에 가라"는 주소(프로토콜 상대 URL)로 읽기 때문이다.
역슬래시를 함께 거절하는 이유는 브라우저의 URL 해석 규칙 때문이다.
`/\evil.com`처럼 슬래시 하나 뒤에 역슬래시를 붙인 값은 앞의 검사를 통과하지만, 브라우저는 역슬래시를 슬래시로 읽어 `//evil.com`으로 간다footnote:[WHATWG URL Standard의 special scheme 처리 규칙. https://url.spec.whatwg.org/#special-authority-ignore-slashes-state[URL Standard §4.4]].
로그인 시작과 콜백, `requireUser`가 전부 이 함수를 거친다.
네 줄짜리 함수지만 없으면 보안 구멍이 되는 코드다.
...
돌아가는 주소에 붙인 `?signed_out=1`은 서비스 워커가 이 사용자를 위해 캐시해 둔 화면을 지우라는 신호인데, 그 캐시 이야기는 {ch-caching}에서 다룬다.
서버 폐기 호출이 돌려준 오류를 `_`로 버린 것은 대충 넘어간 것이 아니라 의도한 판단이다.
GoTrue 호출이 네트워크 사정으로 실패하더라도 쿠키 삭제만으로 이 브라우저는 확실히 로그아웃되고, 남은 리프레시 토큰은 쿠키와 함께 브라우저에서 사라져 아무도 꺼내 쓸 수 없는 값이 된다.
앞 절에서 본 대로 Supabase의 리프레시 토큰은 저절로 만료되지 않으므로, 서버 쪽 폐기가 실패한 토큰은 GoTrue 안에 남아 있기는 하다.
다만 그 값을 아는 곳이 어디에도 없으니 실제로 쓰일 길이 없다.
...
전통적인 방식은 서버가 세션을 자기 메모리나 Redis(값을 메모리에 두고 빠르게 꺼내 주는 별도의 저장 서버)에 두고, 쿠키로는 세션 ID만 주고받는 구조다.
상주 서버에서는 세션 ID로 메모리를 한 번 조회하면 검증이 끝나고 그 항목을 지우면 즉시 폐기되므로 좋은 선택이다.
...
서명은 발급자만 찍을 수 있지만 진짜인지 대조하는 일은 누구나 할 수 있는데, 그 대조에만 쓰는 열쇠가 공개키(public key)다.
Supabase는 이 대조용 열쇠를 JWKS라는 공개 주소에 걸어 두므로, 우리 서버는 비밀 값을 하나도 두지 않고도 토큰의 진위를 가릴 수 있다.
서명을 공개키로 확인하는 절차는 {ch-token-verification}에서 다룬다.
무상태(stateless)라서 요청이 몰려 인스턴스가 몇 개로 늘어나도(수평 확장) 걸리는 것이 없고, 콜드스타트 때 세션 저장소 연결을 기다릴 필요도 없다.
...
대가도 정직하게 짚어 두자.
발급된 토큰은 만료 전까지 유효해서 즉시 강제로 폐기하기 어렵다.
...
`internal/config/config.go`에서 운영 모드로 판정한 뒤의 검증 부분을 보자.
...
Supabase에는 브라우저가 테이블에 바로 요청을 보낼 수 있는 통로(PostgREST)도 딸려 온다.
Flashcard는 이 통로를 잠가 두었다.
모든 테이블에 RLS를 켜 두고 정책을 하나도 만들지 않아서, anon key로 PostgREST에 접근해도 아무 행도 읽거나 쓸 수 없다({ch-supabase-db}에서 그 잠금 장치를 다룬다).
...
이 허용 목록이 없다면 공격자가 `redirect_to`에 자기 서버를 적어 인가 코드를 자기 쪽으로 배달시킬 수 있다.
그러니 등록을 잊어 로그인이 막히는 불편은 보안 장치가 제대로 작동한다는 증거이기도 하다.
...
에이전트를 쓰되, 고칠 파일을 지시에서 `internal/auth`와 `internal/web/cookies.go`처럼 이름으로 못 박고 결과 diff는 한 줄씩 정독한다.

`CLAUDE.md`에는 "토큰은 HttpOnly 쿠키로만 다루고 브라우저 자바스크립트에 노출하지 않는다", "인증 검증 로직은 `internal/auth`에만 둔다", "리다이렉트 대상은 상대 경로만 허용한다"를 적어 둘 만하다.
전부 이 장에서 공격과 방어가 짝지어져 있던 규칙들이라, 에이전트의 "더 깔끔한" 수정이 방어 장치를 걷어내는 사고를 막는다.

인증 코드를 고친 뒤에는 관점을 바꿔 에이전트에게 공격자 역할을 시켜 보자.
"이 변경에서 토큰 탈취, CSRF, 열린 리다이렉트 가능성을 점검해 달라"는 검토 지시는 사람의 정독을 대체하지는 못한다.
다만 이 장에서 공격과 방어를 짝지어 둔 목록이 점검표가 되므로, 정독에서 빠뜨린 항목을 그 목록에 대고 다시 확인하는 보완으로는 쓸 만하다.
...
다만 토큰의 속까지 몰라도 로그인은 이 장까지로 완성이므로, 원리보다 진도가 급한 독자는 {ch-token-verification}을 건너뛰고 {ch-supabase-db}으로 바로 가도 된다.
