...
비밀번호를 받지 않는 이 앱에 한정해서 "Google과 GitHub 로그인은 그대로 두고 Supabase Auth만 걷어 내면 무엇이 남는가"를 따진 목록은 {ch-alternatives}에 있다.
그 목록에서 가장 큰 변화는 토큰의 발급자가 우리가 된다는 것인데, 그것이 무엇을 뜻하는지는 이 장의 "무상태" 절을 읽고 나면 분명해진다.
...
큰 그림은 그림 25에 있다.
세로줄 네 개는 각각 브라우저, 우리 Go 서버, Supabase의 인증 서버(GoTrue), Google·GitHub이고, 가로 화살표는 요청이 가는 방향이며 ①부터 ⑥까지의 번호가 시간 순서다.
...
방문자가 동의하면 프로바이더는 그 브라우저를 GoTrue로 돌려보내고, GoTrue는 다시 같은 브라우저를 `redirect_to`에 적힌 우리 앱의 `/auth/callback`으로 돌려보내는데, 이때 URL에 `?code=`라는 인가 코드가 붙어 온다(④).
...
Supabase의 리프레시 토큰 자체는 기본 설정에서 만료되지 않고 한 번 쓰면 새 토큰으로 교체될 뿐이며, 세션에 상한을 두는 설정은 Pro 플랜부터 제공된다footnote:[Supabase 문서 "User sessions"의 리프레시 토큰과 세션 수명 설명. https://supabase.com/docs/guides/auth/sessions[User sessions]].
...
전통적인 방식(서버가 세션을 메모리나 Redis에 두고 쿠키로는 세션 ID만 주고받는 구조)은 상주 서버에서는 세션 ID로 메모리를 한 번 조회하면 검증이 끝나고 그 항목을 지우면 즉시 폐기되는 좋은 선택이다.
...
Supabase는 이 대조용 열쇠를 JWKS라는 공개 주소에 걸어 두므로, 우리 서버는 비밀 값을 하나도 갖지 않고도 토큰의 진위를 가릴 수 있다.
...
다만 토큰의 속까지 몰라도 로그인은 이 장까지로 완성이므로, 원리보다 진도가 급한 독자는 {ch-token-verification}을 건너뛰고 {ch-supabase-db}로 바로 가도 된다.
