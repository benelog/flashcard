...
이 장의 나머지는 저 몇 줄짜리 미들웨어의 자리를 운영에서 무엇이 대신해야 하는지 다룬다.
...
셋째, 토큰 수명 관리다.
액세스 토큰(access token)의 발급·만료·갱신, 리프레시 토큰(refresh token)의 회전과 폐기, 로그아웃 시 무효화까지, 토큰이 언제 생기고 언제 바뀌고 언제 못 쓰게 되는지의 규칙 전체를 직접 운영해야 한다.
...
자체 호스팅은 VPS(Virtual Private Server), 곧 매달 요금을 내고 빌려 쓰는 가상 서버에 PostgreSQL과 인증 서버를 직접 올리는 방식이고, Supabase 자체가 오픈소스라서 통째로 직접 올리는 것도 가능하다.
...
실패 분기는 원인별로 나뉘어 있어, GoTrue가 `?error=`로 알린 실패, 인가 코드의 부재, verifier 쿠키의 만료, 교환 호출의 실패를 각각 따로 처리한다.
어느 분기든 방문자에게는 오류 메시지를 플래시 쿠키에 담아 로그인 화면으로 되돌리고, 원인은 `log.Printf`로 서버 로그에 남긴다.
...
예를 들어 악성 사이트에 숨겨 둔 폼이 우리 앱의 `/decks/:slug/delete`로 POST를 보내도, 쿠키가 빠진 채 도착하므로 서버에게는 익명 방문자의 요청일 뿐이다.
...
모든 페이지 요청이 이 미들웨어를 지나며, 미들웨어가 방문자의 신원을 확인한다.
