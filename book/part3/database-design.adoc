...
스마트 덱으로 시작한 세션은 그 시점의 규칙을 `smart_rule`에 JSON 텍스트 스냅샷으로 남겨, 규칙이 나중에 수정되거나 삭제돼도 "그때 무엇을 학습했는지"가 보존된다.
...
그 쿼리의 대상이 `cards_with_stats` 뷰(view)로, 카드 내용과 SRS 상태를 조인하고 시도 횟수(`attempts`)와 오답률(`error_rate`)을 파생 칼럼으로 계산해 둔다.
...
`/cards/42` 같은 URL에서는 전체 카드 수가 드러나고 이웃 ID도 추측할 수 있지만, UUID는 추측이 불가능해 ID가 API 경로에 노출되는 앱에서 안전하다.
...
그런데 같은 조건이라도 무엇을 조회하느냐에 따라 이 인덱스가 받는 몫이 달라진다.
...
두 열을 한 테이블에 나란히 둔 이 복합 인덱스가 조건 둘을 한꺼번에 받는 쪽은 `card_srs`를 직접 보는 `DueCount`이고, 뷰를 거치는 목록 조회에는 조인이 한 단계 더 붙는다.
...
태그 검색을 받치는 인덱스도 없는데, 앞서 본 `json_each` 검색은 `cards_user_idx`로 그 사용자의 카드를 모은 뒤 카드마다 태그 배열을 펼쳐 보는 경로라 읽는 양이 그 사람의 카드 수에 비례한다.
카드 5,000장을 넣은 임시 SQLite 파일에서 재 보면 태그 하나로 카드 20장을 고르는 데 이 글을 쓰는 컴퓨터에서 3ms 안팎이 걸려, 한 사람의 카드 수백~수천 장 규모에서는 인덱스 없이 둔다.
...
첫째, 모든 값이 UTC 기준이다.
저장은 하나의 기준 시각으로 하고 해석만 사용자별로 바꾼다는 원칙으로, 서울에서 쓰던 DB 파일을 뉴욕에서 열어도 값의 의미가 달라지지 않는다.
