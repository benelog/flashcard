...
빠뜨린 엔티티는 나중에 표 하나를 더 만드는 것으로 끝나지 않고 이미 쌓인 데이터를 옮겨 심는 일까지 부르므로, 이 단계에서 요구사항을 꼼꼼히 읽는다.
====
...
<p class="fc-caption">그림 15 일곱 테이블의 관계. 실선은 <code>on delete cascade</code>인 소유 관계, 점선은 <code>on delete set null</code>인 참조 관계다. 모든 표에 있는 <code>user_id</code>에서 <code>profiles</code>로 가는 선은 생략했다.</p>
++++
