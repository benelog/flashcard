...
카드 종류 `card_type`이 네 값 중 하나만 갖도록 지키는 `check` 제약은 뒤의 제약 절에서 짚고, `decks`의 `share_slug`·`shared_at`·`seq`가 받치는 덱 공유와 URL slug 설계는 다음 {ch-domain-model}에서 다룬다.
...
{ch-requirements}에서 정리했듯 재도전 라운드의 판정은 기록만 남기고 SRS에는 반영하지 않는데, 그 구분이 컬럼 하나로 스키마에 새겨져 있어, 정답률을 세는 통계 쿼리가 `is_retry = 0` 조건으로 첫 판정만 골라낸다.
