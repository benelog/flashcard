...
운영 PostgreSQL에는 값 목록을 아예 타입으로 만들어 버리는 enum이 있어 같은 울타리를 타입 수준에서 치는데, enum은 값을 더할 때는 `alter type ... add value` 한 줄이면 되지만 값을 빼는 문장은 없어 타입을 새로 만들어 갈아 끼워야 한다는 트레이드오프가 있다(이 앱도 카드 종류에 `concept`를 더할 때 그 한 줄을 썼고, 그 마이그레이션은 {ch-migrations}에서 본다).
