...
이 장은 그 문법 위에서, 화면 열두 장과 조각 여럿을 거느린 앱이 템플릿을 어떻게 조직하는지를 읽는다.
...
페이지 파일 12개(`pages/*.html`), 모든 페이지가 공유하는 공통 레이아웃 1개(`layout.html`), 여러 페이지가 나눠 쓰는 조각 3개(`partials/*.html`)가 전부다.
...
페이지 파일마다 이 공통 마크업을 복사해 두면 고칠 때마다 12곳을 손봐야 하므로, 공통 마크업은 `layout.html` 한 파일에만 쓰고 페이지는 저마다의 본문만 쓴다.
...
그리고 `pages/` 아래의 페이지 파일 12개가 전부 `{{define "content"}}`로 시작한다.
레이아웃이 액자라면 각 페이지는 액자에 끼우는 그림이고, `content`라는 이름이 끼우는 자리다.
최소 앱에서는 화면 파일마다 `<!doctype html>`부터 다시 적었는데, 화면이 열둘로 늘면 그 반복이 상단 바 한 줄을 고치려고 파일 열두 개를 여는 일이 된다.
...
이번 장에서는 {ch-template-hello}에서 익힌 템플릿 문법 위에서, 화면 열두 장짜리 앱이 템플릿을 조직하는 방법을 살펴봤다.

구조는 액자와 그림이다.
`layout.html`이 페이지 본문(`content`)을 감싸고, 페이지 파일 12개가 같은 이름을 쓰는 데서 오는 충돌은 base를 페이지마다 복제하는 Clone 패턴으로 풀었으며, 템플릿에 부족한 계산력은 FuncMap으로 메웠다.
