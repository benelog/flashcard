...
페이지마다 같은 뼈대를 씌우면서 이름 충돌을 피하는 Clone 패턴, 함수를 빌려주는 FuncMap, 사용자 입력을 안전하게 다루는 자동 이스케이프, 템플릿을 실행 파일에 싣는 embed, 그리고 페이지 핸들러의 흐름과 템플릿 테스트까지 차례로 짚는다.

== 완성본의 템플릿 지도

먼저 파일 구성이다.
Flashcard의 템플릿은 `internal/web/templates/` 아래에 있고, 세 갈래로 나뉜다.
페이지 파일 12개(`pages/*.html`), 모든 페이지가 공유하는 공통 뼈대 1개(`layout.html`), 여러 페이지가 나눠 쓰는 조각 3개(`partials/*.html`)가 전부다.
...
페이지 파일마다 이 뼈대를 복사해 두면 고칠 때마다 12곳을 손봐야 하므로, 뼈대는 `layout.html` 한 파일에만 쓰고 페이지는 저마다의 본문만 쓴다.
