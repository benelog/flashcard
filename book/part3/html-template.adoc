...
이 장은 그 문법 위에서, 화면 열세 장과 조각 여럿을 거느린 앱이 템플릿을 어떻게 조직하는지를 읽는다.
...
Flashcard가 쓰는 마크다운 변환기는 goldmark라는 라이브러리인데, 원문에 섞인 생 HTML을 결과에 그대로 통과시킬지 말지를 옵션 하나로 정한다.
그 옵션을 켜지 않으면 사용자가 `<script>`를 적어 넣어도 변환 과정에서 지워지고, 결과에 남는 것은 goldmark 자신이 만든 태그뿐이다.
