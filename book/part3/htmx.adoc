= htmx 실전 패턴: 조각 응답과 무상태 학습 화면

{ch-html-css}에서 화면의 구조와 모양을 만드는 HTML·CSS를, {ch-go-templates}에서 서버가 데이터를 채워 HTML을 완성해 내려보내는 html/template을 살펴봤다.
htmx의 기본기는 이미 손에 있다.
{ch-htmx-hello}에서 최소 앱의 카드 추가·삭제를 만들며 `hx-post`·`hx-target`·`hx-swap` 세 속성과 `HX-Request` 분기, 조각을 돌려주는 서버까지 다뤘다.

그 기본기만으로는 답이 나오지 않는 화면이 완성본에 있다.
Flashcard의 학습 화면을 떠올려 보자.
카드를 뒤집고, 맞았는지 틀렸는지 버튼을 누르면, 곧바로 다음 카드가 나타나는 리듬이 이 앱의 핵심 경험이다.
채점이 이어지는 동안 서버는 남은 카드와 라운드, 점수 같은 진행 상태를 알아야 하는데, 잠들었다 깨어나는 서버리스 환경({ch-vercel})에서는 그 상태를 서버 메모리에 둘 수 없다.
"요청 하나에 조각 하나"로 끝나던 최소 앱에는 없던 문제다.

이번 장에서는 완성본이 htmx를 앱의 규모로 키우는 설계를 다룬다.
먼저 최소 앱에서 만든 것과 같은 카드 삭제 버튼을 완성본 쪽과 나란히 놓아 무엇이 달라졌는지 확인하고, 페이지와 조각이 같은 템플릿을 공유하는 응답 구조(`renderPartial`)를 읽는다.
이어서 이 장의 중심인 무상태 학습 화면(진행 상태를 hidden 필드에 실어 왕복시키는 설계), 응답 하나로 여러 자리를 갈아 끼우는 `hx-swap-oob`, 삭제 후 목록으로 이동시키는 `HX-Redirect`를 실제 코드로 짚어 보겠다.
마지막으로 htmx를 쓰더라도 끝내 남는 최소한의 자바스크립트(`app.js`)를 덩어리별로 해부한다.

== 다시 보는 두 갈래: 이 앱은 어디까지 htmx로 가는가

{ch-htmx-hello}에서는 htmx가 무엇인지 익히는 것이 목적이라, SPA라는 다른 길은 이름만 짚고 지나갔다.
완성본을 해부하기 전에, 이 선택이 어떤 조건 위에 서 있는지 앱 전체의 눈으로 정리하고 가자.
...
{ch-htmx-hello}에서 익힌 htmx가 이 계열의 대표다.
...
`htmx.min.js` 파일을 외부 서버 대신 저장소에 복사해 두는 이유는 {ch-htmx-hello}에서 봤다(이 방식을 벤더링, vendoring이라 부른다).
...
== 카드 삭제 버튼, 최소 앱과 무엇이 다른가

{ch-htmx-hello}에서 만든 것과 같은 버튼부터 보자.
덱 상세 화면의 카드 목록에서 카드 한 장을 삭제하는, 목록에서 그 줄만 사라지게 하는 버튼이다.
같은 일을 하는 완성본의 실물을 옆에 놓으면, 기본기 위에 무엇이 더해졌는지가 드러난다.
...
`hx-confirm`, `hx-target="closest li"`, `hx-swap="outerHTML"`은 {ch-htmx-hello}의 삭제 버튼에 붙어 있던 속성 그대로다.
서버가 빈 응답을 주면 `<li>` 전체가 빈 것으로 교체되어 그 줄이 목록에서 사라지는 것도 같다.

다른 것은 요청을 보내는 방식이다.
최소 앱은 `hx-delete`로 DELETE 요청을 보냈지만, 완성본은 `hx-post`로 `/cards/{{.ID}}/delete`라는 POST 주소를 부른다.
DELETE는 브라우저의 폼이 보낼 수 없는 메서드다({ch-htmx-hello}).
지우는 요청을 전부 POST 주소로 받아 두면 htmx가 없는 환경에서도 같은 주소를 일반 폼 제출로 쓸 수 있고, 완성본의 삭제 라우팅(카드·덱·스마트 덱)은 실제로 전부 이 모양이다.
...
`HX-Request` 분기는 {ch-htmx-hello}에서 본 그대로다.
달라진 것은 htmx가 아닌 쪽의 대접이다.
최소 앱의 삭제 버튼은 htmx가 없으면 눌러도 아무 일도 일어나지 않았지만, 완성본은 일반 요청에 플래시 메시지({ch-go-templates})를 남기고 이전 화면으로 돌려보내는 온전한 폼 제출 경로를 갖춘다.
...
속성 세 개와 `HX-Request` 헤더라는 기본기는 {ch-htmx-hello}에서 익힌 그대로이고, 완성본은 그 위에 설계를 얹는다.
