...
{ch-html-css}에서 화면의 구조와 모양을 만드는 HTML·CSS를, {ch-go-templates}에서 서버가 데이터를 채워 HTML을 완성해 내려보내는 html/template을 살펴봤다.
...
htmx가 이 계열의 대표다.
{ch-go-templates}에서 만든 서버 렌더링 구조가 그대로 연장되므로, 화면을 그리는 로직은 여전히 Go 한 곳에만 있다.
...
페이지 한 장을 그릴 줄 아는 서버라면 그 일부인 조각도 그릴 수 있으므로, {ch-go-templates}까지 읽은 서버에 더할 것이 많지 않다.
...
완성본은 여기서 한 걸음 더 나아가, {ch-go-templates}에서 본 embed로 이 파일을 Go 바이너리 안에 넣었다.
...
헤더가 없으면, 즉 htmx 없이 일반 폼으로 제출된 요청이면 플래시 메시지({ch-go-templates})를 남기고 이전 화면으로 돌려보낸다.
...
{ch-go-templates}에서 본 `render`는 layout 템플릿부터 실행해 `<html>`로 시작하는 페이지 전체를 만들었다.
...
`partials`가 `templates/partials/` 아래 파일들로 만들어진다는 것은 {ch-go-templates}의 템플릿 파싱 코드에서 봤다.
...
흐름을 따라가 보자.
`stateFromForm`이 폼 값에서 `studyState`를 다시 조립하고, 큐의 첫 카드를 꺼내 채점 결과를 데이터베이스에 기록한다(이것이 서버에 남는 유일한 것으로, {ch-database}에서 설계한 리뷰 기록이다).
...
외부 API 호출과 응답 가공이 Go 코드에 있으므로 브라우저 자바스크립트가 필요 없고, 응답을 두 줄 요약으로 다듬는 로직(`summarizeEntries`)은 {ch-go-testing}에서 본 방식대로 Go 테스트로 검증할 수 있다.
...
서버가 평소처럼 303 리다이렉트({ch-go-templates}의 PRG 패턴에서 썼던, 브라우저를 다른 주소로 이동시키는 응답)를 보내면 곤란한 일이 생긴다.
...
{ch-html-css}에서 문서(HTML)와 스타일(CSS)을, {ch-go-templates}에서 서버가 데이터를 채우는 html/template을, 이번 장에서 새로고침 없는 동적 화면을 만드는 htmx를 살펴봤다.
