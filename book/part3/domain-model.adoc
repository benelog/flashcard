...
둘 다 요구사항에서 곧바로 컬럼이 나오지 않는다는 공통점이 있다.
도메인의 규칙을 먼저 정하고, 그 규칙이 데이터로 어떻게 남아야 하는지를 역산해야 컬럼이 정해진다.
...
맞음/틀림 판정 하나를 받아 위 컬럼들의 다음 값과 다음 복습 시각을 계산하는 `Grade`가 그것이고, 시그니처는 `func Grade(s State, correct bool, now time.Time) (State, time.Time)`이다.
...
스키마를 설계할 때 "알고리즘이 다음 계산에 쓰는 최소 상태가 무엇인가"를 물으면 컬럼 목록이 저절로 나온다.
...
그래서 요구사항의 미묘한 규칙 하나가 분기 하나와 컬럼 하나로 구현됐다.
...
`card_srs_user_due_idx`는 `card_srs`의 `user_id`에 걸린 인덱스인데, 뷰가 노출하는 `user_id`는 `cards` 쪽 컬럼이라 DB가 이 인덱스를 쓸 자리로 보지 않는다.
...
Base62(대소문자 구분)보다 표현력은 낮지만 대소문자를 구분하지 않아 사용자가 slug를 입으로 전달하거나 손으로 입력할 때 혼동이 없다.
...
조회 쪽은 `internal/litestore/decks.go`의 `GetDeckBySlug`가 `DecodeDeckSlug`로 slug를 seq로 되돌린 뒤 `where d.user_id = ? and d.seq = ?` 조건으로 찾는다.
...
"재도전 라운드의 답은 SRS 채점에 반영하지 않는다"처럼 요구사항 문장 하나가 컬럼 하나로 굳는 대목에서는, 그 문장을 지시문에 그대로 실어 준다.
규칙을 빼고 "복습 기록 테이블을 만들어 달라"고만 하면 에이전트는 흔한 설계를 골라 오고, 그 설계에는 `is_retry` 같은 컬럼이 없다.
...
SRS 상태는 알고리즘에 필요한 최소 상태로 컬럼을 구성했고, 복습 기록은 트랜잭션으로 로그 삽입과 상태 갱신을 원자적으로 처리한다.
재도전 라운드를 SRS에서 제외하는 도메인 규칙은 `is_retry` 컬럼 하나로 스키마에 새겨졌다.

덱 slug는 `max(seq)+1`로 채번한 순번을 곱셈 순열로 뒤섞은 Base36 4자 코드로, 짧은 URL과 순번 은닉을 저장 컬럼 추가 없이 얻었다.
