...
{ch-database}에서 세운 테이블들은 대체로 어느 앱에나 있을 법한 것들이었다.
...
`due_at <= ?`의 자리에는 `fmtTime`으로 포맷한 현재 시각 텍스트가 들어가며, {ch-database}의 시각 절에서 본 고정 폭 표기 덕분에 이 문자열 비교가 정확히 시간 비교로 동작한다.
...
먼저 그 코드가 저장소 어디에 어떤 원칙으로 놓여 있는지 지도를 그리고, {ch-go}에서 방금 본 테이블 위의 핵심 로직을 따라가게 된다.
