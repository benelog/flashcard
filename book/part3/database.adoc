...
"오답률 0.4 이상" 같은 규칙 조건과 store 계층(DB를 읽고 쓰는 Go 코드로, {ch-go}과 {ch-app-assembly}에서 다룬다)의 카드 조회가 모두 이 뷰 하나를 바라보므로, 오답률 계산식이 한 곳에만 존재한다.
...
운영 PostgreSQL의 전용 타입(uuid, timestamptz, jsonb, 배열, enum)은 같은 의도를 타입 자체로 표현하는데, 그 대응은 {ch-supabase-db}에서 다룬다.
