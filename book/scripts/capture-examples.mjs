...
//
// 원고의 연습용 예제(language-basic/)를 headless Chrome으로 열어 찍는다.
// 앱 캡처(capture-screenshots.mjs)와 달리 로그인도 서버도 필요 없다.
//
// 사용 절차:
//   1. book/ 에서 실행: node scripts/capture-examples.mjs
//   2. 합성·축소(ImageMagick)까지 스크립트가 함께 처리해
//      book/public/screenshots/ 에 아래 세 장을 남긴다.
...
const EXAMPLES = path.join(HERE, '..', '..', 'language-basic', 'html');
