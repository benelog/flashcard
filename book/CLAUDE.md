...
2부 **코딩 첫걸음: 문법에서 첫 웹 앱까지**(6~13장)은 HTML·CSS, Go, SQL의 최소 문법을 연습용 예제(`language-basic/`)로 익히고, 이어서 최소 플래시카드 앱(`flashcard-basic/`)을 재료로 Go에서 SQLite 쓰기, Gin, htmx의 기초를 다룬다.
3부 **완성본 앱 해부**(14~26장)는 완성본 앱의 설계 원칙을 먼저 읽은 뒤 실제 코드를 데이터에서 화면 순으로 읽고 앱을 통째로 띄우는 테스트로 그 층들의 배선까지 확인하며,
4부 **인터넷에 공개하기: 버전 관리에서 배포와 인증까지**(27~35장)는 버전 관리·품질 게이트·배포·인증·운영 DB를,
