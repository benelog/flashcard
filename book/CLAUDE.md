...
2부 **세 언어 첫걸음과 첫 웹 앱**(6~13장)은 HTML·CSS, Go, SQL의 최소 문법을 연습용 예제(`language-basic/`)로 익히고, 이어서 최소 플래시카드 앱(`flashcard-basic/`)을 재료로 Go에서 SQLite 쓰기, Gin, htmx의 기초를 다룬다.
3부 **앱 코드 해부**(14~26장)는 이 앱의 설계 원칙을 먼저 읽은 뒤 실제 코드를 데이터에서 화면 순으로 읽고 앱을 통째로 띄우는 테스트로 그 층들의 배선까지 확인하며,
4부 **세상에 공개하기**(27~35장)는 버전 관리·품질 게이트·배포·인증·운영 DB를,
...
| 11 | Go에서 SQLite 쓰기: database/sql로 저장하고 꺼낸다 | `part2/go-sqlite.adoc` |
...
| 16 | 도메인을 표에 담기: 학습 상태와 덱 주소 | `part3/domain-model.adoc` |
| 17 | 코드는 어디에 있는가: 패키지 지도와 앱의 시작점 | `part3/code-map.adoc` |
| 18 | 핵심 로직 읽기: 간격 반복 알고리즘과 도메인 코드 | `part3/go.adoc` |
| 19 | Go 테스트와 품질 검사 도구 | `part3/go-testing.adoc` |
| 20 | Gin으로 만드는 HTTP API: 라우팅·바인딩·미들웨어 | `part3/gin.adoc` |
| 21 | 앱을 조립하기: 계층 분리와 두 진입점 | `part3/app-assembly.adoc` |
| 22 | 화면의 구조: 템플릿 삼층과 스타일 시스템 | `part3/html-css.adoc` |
| 23 | html/template으로 만드는 화면 | `part3/go-templates.adoc` |
| 24 | htmx: 자바스크립트 없이 만드는 동적 화면 | `part3/htmx.adoc` |
...
| 28 | 품질 게이트: 훅과 서브에이전트 | `part4/quality-gates.adoc` |
...
| 34 | Supabase 데이터베이스: pgx 연결과 커넥션 풀 | `part4/supabase-db.adoc` |
| 35 | 마이그레이션: 운영 중에 스키마를 고친다 | `part4/migrations.adoc` |
| 36 | 환경 변수와 시크릿: 값이 두 벌이 된다 | `part5/env-secrets.adoc` |
| 37 | PWA: 설치되는 앱으로 만들기 | `part5/pwa.adoc` |
| 38 | 캐시와 서비스 워커: 낡은 화면이 남지 않게 | `part5/caching.adoc` |
| 39 | 무료 티어 운영과 한도 관리 | `part5/free-tier.adoc` |
