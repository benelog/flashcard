= 부록 배포 준비: Supabase·Google·GitHub·Vercel 설정
...
23장부터 28장까지는 이 구성이 왜 이렇게 짜였는지를 다뤘다.
...
첫째 관문은 Supabase에서 그 프로바이더(provider, 로그인을 대신 처리해 주는 Google·GitHub 같은 서비스. 자세한 설명은 24장)가 켜져 있어야 지난다.
...
이 책은 개발용과 운영용 DB를 완전히 나눠 쓴다(26장).
...
클라이언트를 만들면 Client ID와 Client Secret이 나온다(둘의 차이는 24장).
이 값을 Supabase로 가져간다.
...
뒤엣것은 로컬에서 개발 DB에 붙어 로그인을 시험할 때 쓴다(`run_dev.sh`, 28장).
...
20장에서 개념으로 정리한 첫 push를 여기서 실제로 한다.
...
공개(Public)와 비공개(Private) 중 무엇을 골라도 28장의 규칙은 같다.
비밀 값은 커밋하지 않는다.
...
push가 끝나면 20장의 브랜치 정책대로 운영용 release 브랜치를 만들어 함께 올려 둔다.
...
`vercel.json`의 `"framework": null`이 그 값을 덮어쓰기 때문이다(23장).

이 책의 브랜치 정책은 main을 개발용, release를 운영용으로 삼는다(20장).
...
Production 스코프에는 운영 프로젝트 값을, Preview 스코프에는 개발 프로젝트 값을 넣는다(이유는 28장).
이렇게 해야 개발 배포는 개발 DB에, 운영 배포는 운영 DB에 붙는다.
세 번째 스코프인 Development 칸은 비워 둔다.
이름만 보면 개발 값이 들어갈 자리 같지만, 이 스코프는 `vercel dev`라는 로컬 실행 명령이 읽는 자리라 이 책의 구성에서는 쓰이지 않는다(23장).
