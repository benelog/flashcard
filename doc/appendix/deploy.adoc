...
14장에서 개념으로 정리한 첫 push를 여기서 실제로 한다.
...
공개(Public)와 비공개(Private) 중 무엇을 골라도 18장의 규칙은 같다.
비밀 값은 커밋하지 않는다.
...
push가 끝나면 14장의 브랜치 정책대로 운영용 release 브랜치를 만들어 함께 올려 둔다.
...
이름만 보면 개발 값이 들어갈 자리 같지만, 이 스코프는 `vercel dev`라는 로컬 실행 명령이 읽는 자리라 이 책의 구성에서는 쓰이지 않는다(16장).
