...
`doc/` 아래 원고(preface.adoc, how-to-read.adoc, requirements.adoc, part1/~part5/, appendix/)를 쓰거나 고칠 때 지키는 규칙이다.
...
`part5/env-secrets.adoc`(29장)은 소유 상자가 없다(환경 변수·연결 문자열 상자는 27장 소유를 따른다).

**동음이의어 주의.** 같은 말이 다른 뜻으로 쓰이는 곳은 그 자리에서 한 구절로 구분해 준다. 모르는 말은 독자가 멈추지만, 아는 줄 알았던 말은 조용히 오해로 굳는다.

- **훅**: 22장의 훅은 Claude Code가 자동 실행하는 스크립트이고, React 같은 프런트엔드 프레임워크의 훅(상태 관리 함수)과는 다른 것이다(22장 상자에서 한 문장으로 짚는다).
- **세션**: 네 가지 뜻으로 쓰인다. 3장은 Claude Code의 대화 단위, 18장은 카드 한 벌을 도는 한 번의 학습(`study_sessions`), 25장은 로그인 상태, 27장은 DB에 접속해 있는 연결 하나다. 18·25·27장에서 각각 다른 뜻임을 짚는다.
- **컨텍스트**: 3장은 에이전트에게 건네는 대화 내용, 17장은 템플릿이 지금 보고 있는 데이터 한 덩어리다(17장에서 구분).
- **커넥션 풀과 커넥션 풀러**: 이름이 한 글자 차이지만 다른 물건이다. 풀은 우리 서버가 자기 안에 쥐고 돌려쓰는 연결 묶음(pgxpool), 풀러는 DB 앞에 서서 중개하는 별도의 서버(Supavisor)다(27장 상자에서 구분).
- **메서드**: 9장은 구조체에 딸린 함수(Go의 메서드), 15장은 HTTP 요청의 종류(GET, POST)다(9장 상자에서 구분).
- **토큰**: 3장은 LLM이 글을 쪼개 세는 단위, 25장은 로그인 상태를 증명하는 문자열이다(25장 상자에서 구분).
- **인가**: 10장은 그 사람이 이 일을 해도 되는지 앱 안에서 확인하는 권한 검사이고, 25장의 인가 코드·인가 화면·인가 주소는 사용자가 프로바이더 화면에서 앱의 로그인 요청을 허가하는 OAuth 절차다(25장 본문에서 한 문장으로 구분).
...
- 헤더에 계정 이메일 대신 "로컬 모드"가 뜬다. 로컬 모드는 5장과 20장에서 설명하는 실행 방식이라 그대로 싣는다.
...
- 현재 실린 그림: 그림 1 덱 상세, 그림 2 학습 3단계, 그림 3 홈·복습 큐, 그림 4 통계·공유(모두 requirements), 그림 9 카드 입력 화면(알약 라디오), 그림 10 밝은 테마와 어두운 테마(16장).
...
1. 문어체 평서형 **'~다'** 로 끝맺는다. '~입니다'체 금지. 용어 상자 안에서도 '~다'체.
   ...
   - 이 예외는 서문에만 해당한다. 본문 원고(how-to-read, requirements, part1/~part5/, appendix/)에서 '~습니다'체를 발견하면 고친다.
