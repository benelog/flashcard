...
다섯 부 36장으로 나뉜다(2026-07-25 전면 재편, 같은 날 분량 균등화 재편과 도입·서문 재배치).
...
3부 **앱 코드 해부**(11~22장)는 실제 코드를 데이터에서 화면 순으로 읽고 앱을 통째로 띄우는 테스트로 그 층들의 배선까지 확인하며,
4부 **세상에 공개하기**(23~31장)는 버전 관리·품질 게이트·배포·인증·운영 DB를,
5부 **안정적으로 오래 운영하기**(32~36장)는 값 관리·PWA·캐시·무료 티어·다음 공부를 다룬다.
...
완성본 앱의 클론·실행 체험은 5장에 두고, 그 원리 해부는 22장에 둔다.

**3부는 문법을 다시 가르치지 않는다(2026-07-25 재편).**
같은 문법을 두 번 설명하면 2부가 무의미해지고 3부는 재탕이 된다. 3부의 장들은 문법 항목이 아니라 **이 앱의 구조와 이 앱이 푸는 문제**로 절을 나눈다.

- 13장: 패키지 지도, 의존 방향, 앱의 시작점(코드가 어디에 있는가)
- 14장: 간격 반복, 스마트 덱 규칙, 에러 번역, 값의 부재, 반복 제거(이 앱이 푸는 문제)
- 18장: 템플릿 삼층, 브라우저 기본 기능으로 끝낸 폼, 스타일시트를 조직하는 네 규칙, 상태를 HTML에 들리는 패턴

문법 이름을 절 제목에 쓰지 않는다(변수·상수·제어문·포인터·슬라이스는 2부의 절 제목이다). 3부에서 문법이 필요하면 그 자리에서 한두 문장으로 짚고 2부를 `{ch-}` 토큰으로 가리킨다.
**연습용 예제도 3부에 두지 않는다.** 제네릭 문법 소개(firstString/sum 예제)가 14장에 있던 것을 9장으로 옮긴 것이 그 예다.
토큰 검증(29장)처럼 "따라 하기에 필수는 아닌 원리 심화"는 별도 장으로 분리해 건너뛸 수 있게 한다.
테스트도 두 자리로 나눠 둔다. 15장은 함수 하나를 부르는 단위 테스트와 품질 검사 도구를, 21장은 앱을 통째로 띄워 화면과 API의 배선을 확인하는 통합 테스트를 다룬다. 21장이 검증하는 것(PRG 리다이렉트, htmx 헤더, hidden 필드 왕복)을 16·19·20장에서 먼저 배우므로 이 순서를 뒤집지 않는다.

**장 하나가 너무 커지면 나눈다.** 큰 장은 대개 절 경계에 주제의 이음매가 이미 있다. 16·17장(Gin/조립), 11·12장(테이블 설계/도메인 설계), 26·27장(Vercel 선택/설정), 33·34장(PWA 설치/캐시)이 그렇게 갈라 둔 짝이다. 대안 기술 서술을 본문에서 부록 A로 뺀 것(2026-07-25)도 같은 이유다. 분량은 `node scripts/chapter-stats.mjs`로 재고, 결과와 진단은 `chapter-dist.md`에 있다.
...
| 12 | 도메인을 표에 담기: 학습 상태와 덱 주소 | `part3/domain-model.adoc` |
| 13 | 코드는 어디에 있는가: 패키지 지도와 앱의 시작점 | `part3/code-map.adoc` |
| 14 | 핵심 로직 읽기: 간격 반복 알고리즘과 도메인 코드 | `part3/go.adoc` |
| 15 | Go 테스트와 품질 검사 도구 | `part3/go-testing.adoc` |
| 16 | Gin으로 만드는 HTTP API: 라우팅·바인딩·미들웨어 | `part3/gin.adoc` |
| 17 | 앱을 조립하기: 계층 분리와 두 진입점 | `part3/app-assembly.adoc` |
| 18 | 화면의 구조: 템플릿 삼층과 스타일 시스템 | `part3/html-css.adoc` |
| 19 | html/template으로 만드는 화면 | `part3/go-templates.adoc` |
| 20 | htmx: 자바스크립트 없이 만드는 동적 화면 | `part3/htmx.adoc` |
| 21 | 앱을 통째로 띄우는 테스트: 화면과 API를 한 번에 검증하기 | `part3/app-tests.adoc` |
| 22 | 로컬 개발 환경: 무설정 실행의 원리와 개발 도구 | `part3/local-dev.adoc` |
| 23 | Git: 개념과 브랜치 정책 | `part4/git.adoc` |
| 24 | 품질 게이트: 훅과 서브에이전트 | `part4/quality-gates.adoc` |
| 25 | GitHub Actions: 원격 품질 게이트 | `part4/github-actions.adoc` |
| 26 | Vercel: 왜 이 플랫폼이고, 무엇이 배포되는가 | `part4/vercel.adoc` |
| 27 | Vercel 설정과 제약: vercel.json, 콜드스타트, 배포 절차 | `part4/vercel-config.adoc` |
| 28 | Supabase 인증: OAuth 로그인과 쿠키 세션 | `part4/supabase-auth.adoc` |
| 29 | 토큰 검증의 원리: JWT와 JWKS | `part4/token-verification.adoc` |
| 30 | Supabase 데이터베이스: pgx 연결과 커넥션 풀 | `part4/supabase-db.adoc` |
| 31 | 마이그레이션: 운영 중에 스키마를 고친다 | `part4/migrations.adoc` |
| 32 | 환경 변수와 시크릿: 값이 두 벌이 된다 | `part5/env-secrets.adoc` |
| 33 | PWA: 설치되는 앱으로 만들기 | `part5/pwa.adoc` |
| 34 | 캐시와 서비스 워커: 낡은 화면이 남지 않게 | `part5/caching.adoc` |
| 35 | 무료 티어 운영과 한도 관리 | `part5/free-tier.adoc` |
| 36 | 다음 단계: 여기서 더 공부할 것들 | `part5/whats-next.adoc` |
...
- **마커를 달지 않는 경우가 있다.** 한두 줄짜리 발췌는 마커가 코드보다 길어지므로 원고에 그대로 쓴다. 구조체 필드 사이에도 넣지 않는다. gofmt가 주석을 경계로 정렬 그룹을 끊어 필드 정렬이 깨진다(15장이 설명하는 그 세로 정렬이다). 구조체는 통째로 인용한다.
...
`part5/env-secrets.adoc`(32장)은 소유 상자가 없다(환경 변수·연결 문자열 상자는 30장 소유를 따른다).

**동음이의어 주의.** 같은 말이 다른 뜻으로 쓰이는 곳은 그 자리에서 한 구절로 구분해 준다. 모르는 말은 독자가 멈추지만, 아는 줄 알았던 말은 조용히 오해로 굳는다.

- **훅**: 24장의 훅은 Claude Code가 자동 실행하는 스크립트이고, React 같은 프런트엔드 프레임워크의 훅(상태 관리 함수)과는 다른 것이다(24장 상자에서 한 문장으로 짚는다).
- **세션**: 네 가지 뜻으로 쓰인다. 4장은 Claude Code의 대화 단위, 19장은 카드 한 벌을 도는 한 번의 학습(`study_sessions`), 28장은 로그인 상태, 30장은 DB에 접속해 있는 연결 하나다. 19·28·30장에서 각각 다른 뜻임을 짚는다.
- **컨텍스트**: 3장은 에이전트에게 건네는 대화 내용, 19장은 템플릿이 지금 보고 있는 데이터 한 덩어리다(19장에서 구분).
- **커넥션 풀과 커넥션 풀러**: 이름이 한 글자 차이지만 다른 물건이다. 풀은 우리 서버가 자기 안에 쥐고 돌려쓰는 연결 묶음(pgxpool), 풀러는 DB 앞에 서서 중개하는 별도의 서버(Supavisor)다(30장 상자에서 구분).
- **메서드**: 9장은 구조체에 딸린 함수(Go의 메서드), 16장은 HTTP 요청의 종류(GET, POST)다(9장 상자에서 구분).
- **토큰**: 3장은 LLM이 글을 쪼개 세는 단위, 28장은 로그인 상태를 증명하는 문자열이다(28장 상자에서 구분).
- **인가**: 10장은 그 사람이 이 일을 해도 되는지 앱 안에서 확인하는 권한 검사이고, 28장의 인가 코드·인가 화면·인가 주소는 사용자가 프로바이더 화면에서 앱의 로그인 요청을 허가하는 OAuth 절차다(28장 본문에서 한 문장으로 구분).
...
- 헤더에 계정 이메일 대신 "로컬 모드"가 뜬다. 로컬 모드는 5장과 22장에서 설명하는 실행 방식이라 그대로 싣는다.
...
- 현재 실린 그림: 그림 1 덱 상세, 그림 2 학습 3단계, 그림 3 홈·복습 큐, 그림 4 통계·공유(모두 1장 `part1/requirements.adoc`), 그림 9 카드 입력 화면(알약 라디오), 그림 10 밝은 테마와 어두운 테마(18장).
