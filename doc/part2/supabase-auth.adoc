= 21장 Supabase 인증: OAuth와 JWKS 검증

20장에서 Go 서버를 Vercel 서버리스 함수로 배포하는 과정을 살펴봤다.
남은 조각은 사용자 인증과 데이터베이스이고, 둘 다 Supabase가 맡는다.
성격이 꽤 다른 두 이야기라 장을 나눴다.
이 장은 인증을, 다음 22장은 데이터베이스 연결을 다룬다.
...
무료 플랜의 한도와 일시정지·백업 같은 운영 문제는 24장에서 Vercel과 함께 묶어 다룬다.
...
도입 장에서 정한 Flashcard의 비기능 요구사항을 다시 떠올려 보자.
...
다만 무료에는 무료의 규칙이 따라붙는데, 그 한도와 함정은 장 첫머리에서 말한 대로 24장의 몫이다.
...
이 장에서 계속 만날 인가 코드·인가 화면·인가 주소의 "인가"는 사용자가 프로바이더 화면에서 우리 앱의 로그인 요청을 허가하는 OAuth 절차를 가리키는 말로, 4장에서 본 인가(그 사람이 이 일을 해도 되는지 앱 안에서 확인하는 권한 검사)와는 다른 뜻이다.
...
2장의 토큰(LLM이 글을 쪼개 세는 단위)과는 이름만 같을 뿐 다른 말이고, 이 장의 토큰은 로그인 상태를 증명하는 문자열이다.
====
...
PostgREST를 쓰지 않는 이유는 데이터 접근 규칙이 단순 CRUD(20장)가 아니기 때문이다.
...
그러나 Flashcard의 화면은 15장과 16장에서 본 대로 서버에서 렌더링되고, 곧 확인하겠지만 로그인 흐름에도 브라우저 자바스크립트가 필요 없다.
...
악성 스크립트를 우리 페이지 안에 심는 XSS(15장)와는 다른 공격이다.
...
패키지 전역 변수 `jwks`를 `sync.Once`로 한 번만 초기화하는 것은 20장에서 본 서버리스 함수의 생명주기 때문이다.
...
이 함수를 부르는 곳은 둘이다.
11장에서 본 JSON API용 `parseUserID`는 `sub`만 꺼내 사용자 ID를 돌려주고, 웹 계층용 `ParseUser`는 화면에 표시할 이메일 클레임까지 함께 돌려준다.
앞서 `withUser`가 쿠키에서 꺼낸 토큰을 건네던 것이 뒤쪽이다.
...
두 미들웨어가 라우터 조립에서 나뉘어 붙는 곳과 그 이유(공유 덱의 "익명 허용 + 로그인 시 개인화" 요구, 그리고 `Cache-Control: no-store`가 한 세트인 사연)는 11장에서 다뤘다.
...
로그인 그룹 끝의 `EnsureProfile`은 첫 요청에서 사용자의 프로필 행을 만들어 주는 미들웨어로, 11장의 JSON API와 이 웹 페이지 그룹이 함께 쓴다.
...
데이터에 닿는 길은 Go 서버의 직접 연결(22장) 하나뿐이다.
게다가 우리 구성에서는 GoTrue 호출이 전부 서버에서 일어나므로, 이 키가 브라우저에 내려갈 일 자체가 없다.

=== OAuth 앱 등록과 Redirect URL

코드 밖에서 손으로 해야 하는 준비가 두 층 있다.
어느 화면에서 무엇을 누르는지까지의 클릭 절차는 부록 B(Supabase·Google·GitHub·Vercel 설정)에 화면 캡처와 함께 정리해 두었으니, 여기서는 구조만 잡는다.
...
사람이 서비스에 로그인할 때 아이디와 비밀번호를 대는 것처럼, 앱이 프로바이더에게 "나는 그때 등록한 그 앱이다"라고 밝히는 데 쓴다.
...
다음 22장에서는 같은 Supabase의 다른 얼굴, 즉 데이터베이스 연결을 다룬다.
