...
PostgREST를 쓰지 않는 이유는 데이터 접근 규칙이 단순 CRUD(16장)가 아니기 때문이다.
...
그러나 Flashcard의 화면은 11장과 12장에서 본 대로 서버에서 렌더링되고, 곧 확인하겠지만 로그인 흐름에도 브라우저 자바스크립트가 필요 없다.
...
악성 스크립트를 우리 페이지 안에 심는 XSS(11장)와는 다른 공격이다.
...
이 함수를 부르는 곳은 둘이다.
9장에서 본 JSON API용 `parseUserID`는 `sub`만 꺼내 사용자 ID를 돌려주고, 웹 계층용 `ParseUser`는 화면에 표시할 이메일 클레임까지 함께 돌려준다.
...
로그인 그룹 끝의 `EnsureProfile`은 첫 요청에서 사용자의 프로필 행을 만들어 주는 미들웨어로, 9장의 JSON API와 이 웹 페이지 그룹이 함께 쓴다.
