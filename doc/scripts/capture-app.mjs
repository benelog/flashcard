...
//
// 사용 절차:
//   cd doc && node scripts/capture-app.mjs
//
// 필요한 것: go, google-chrome, ImageMagick(convert). 포트 8099를 잠깐 쓴다.
// 만드는 파일(doc/public/screenshots/):
...
execFileSync('go', ['build', '-o', BIN, './cmd/server'], { cwd: REPO, stdio: 'inherit' });
...
function startServer() {
  ...
  const proc = spawn(BIN, [], { cwd: REPO, env, stdio: 'ignore' });
  running.add(proc);
  return proc;
}
