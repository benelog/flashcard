= 11장 데이터베이스 설계: 요구사항에서 테이블로

애플리케이션의 수명은 코드보다 데이터가 길다.
프레임워크는 갈아탈 수 있어도 쌓인 학습 기록은 버릴 수 없기 때문에, 데이터베이스 설계는 앱 전체에서 가장 신중해야 할 결정이다.
관계형 데이터베이스를 고르고 로컬 개발에는 SQLite를, 운영에는 PostgreSQL을 쓰는 이유는 1장에서 정리했고, 테이블과 키·SQL·인덱스의 기초는 10장에서 다뤘다고 전제한다.
이 장에서는 도입에서 정리한 요구사항이 실제 테이블과 인덱스로 구체화되는 과정을, 로컬 모드의 앱이 쓰는 SQLite 스키마를 근거로 따라가 보겠다.
간격 반복(Spaced Repetition) 상태를 저장하는 데이터 모델과 URL slug(주소에 쓰는 짧은 식별 문자열) 설계까지가 이 장의 범위이고, 같은 설계가 운영 PostgreSQL에서 어떤 전용 타입과 장치로 표현되는지는 26장에서 다룬다.
...
인증 시스템의 식별자와 앱 데이터의 식별자를 일치시키면 조인 없이 인증된 사용자 ID로 바로 조회할 수 있다(운영의 인증 흐름은 24장에서 다룬다).
...
덱을 거치지 않고 "이 사용자의 모든 카드"를 자주 조회하기 때문에(due 큐, 스마트 덱), 조인을 한 단계 줄이려는 실용적 비정규화(denormalization, 10장)다.
...
"오답률 0.4 이상" 같은 규칙 조건과 store 계층(DB를 읽고 쓰는 Go 코드로, 13장과 15장에서 다룬다)의 카드 조회가 모두 이 뷰 하나를 바라보므로, 오답률 계산식이 한 곳에만 존재한다.
...
규칙의 값들이 SQL 문자열에 직접 이어 붙지 않고 자리표시자 `?`로 바인딩(binding)되는, 즉 문장 밖에서 값으로 따로 전달되는 것은 10장에서 본 SQL 인젝션 방지 원칙 그대로다.
...
운영 스키마도 같은 전략인데, 그쪽은 uuid 타입에 `gen_random_uuid()` 기본값, 순번에는 identity 컬럼이라는 전용 장치가 있다는 점만 다르다(26장).
...
그래서 두 열은 비워 둘 수 있게, 즉 10장에서 본 NULL을 허용하게 선언했다.
...
이 쿼리는 사용자가 딱 하나로 정해지는 조건(`user_id` 등치) 뒤에 복습 시각이 일정 시점 이전이라는 조건(`due_at` 범위)이 오는 형태라, 10장에서 본 (등치, 범위) 순서 원칙에 정확히 들어맞고 `order by due_at`까지 인덱스 순서로 해결할 수 있다.
...
상태 전이 로직은 `internal/srs/srs.go`에 순수 함수(입력을 바꾸지 않고 계산 결과만 돌려주는 함수로, 13장에서 자세히 다룬다)로 분리되어 있다.
...
다음 상태와 복습 시각, 결과 두 개를 함께 돌려주는 시그니처에서 8장에서 익힌 다중 반환이 실전에 쓰이는 모습도 보인다.
...
"덱마다 순번 하나"라는 같은 설계가 환경의 동시성 수준에 따라 다른 채번 장치로 구현되는 대비로, 운영 쪽 장치는 26장에서 본다.
...
지키는 장치는 26장에서, 고치는 절차는 운영 스키마(`internal/db/migrations/`)와 함께 27장에서 다룬다.
...
4장에서 본 대로, 설계안을 사이에 둔 왕복이 요구사항의 빈틈을 드러낸다.
...
전용 타입과 RLS는 26장에서, 마이그레이션은 27장에서 이어진다.

여기까지가 데이터가 머무는 바닥층이다.
다음 12장부터는 한 층 올라가, 이 테이블들을 실제로 읽고 쓰는 Go 코드를 읽는다.
