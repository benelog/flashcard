...
이 장에서는 도입에서 정리한 요구사항이 실제 테이블과 인덱스로 구체화되는 과정을, 로컬 모드의 앱이 쓰는 SQLite 스키마를 근거로 따라가 보겠다.
간격 반복(Spaced Repetition) 상태를 저장하는 데이터 모델과 URL slug(주소에 쓰는 짧은 식별 문자열) 설계까지가 이 장의 범위이고, 같은 설계가 운영 PostgreSQL에서 어떤 전용 타입과 장치로 표현되는지는 {ch-supabase-db}에서 다룬다.
...
그 차이는 {ch-supabase-db}에서 본다.

`cards`에 `deck_id`뿐 아니라 `user_id`도 중복해 둔 점도 설계 결정이다.
덱을 거치지 않고 "이 사용자의 모든 카드"를 자주 조회하기 때문에(due 큐, 스마트 덱), 조인을 한 단계 줄이려는 실용적 비정규화(denormalization, {ch-database-basics})다.
...
"오답률 0.4 이상" 같은 규칙 조건과 store 계층(DB를 읽고 쓰는 Go 코드로, {ch-go}과 {ch-gin}에서 다룬다)의 카드 조회가 모두 이 뷰 하나를 바라보므로, 오답률 계산식이 한 곳에만 존재한다.
...
운영 스키마도 같은 전략인데, 그쪽은 uuid 타입에 `gen_random_uuid()` 기본값, 순번에는 identity 컬럼이라는 전용 장치가 있다는 점만 다르다({ch-supabase-db}).
...
그 차이와 실제로 치른 비용은 {ch-supabase-db}에서 확인한다.
...
상태 전이 로직은 `internal/srs/srs.go`에 순수 함수(입력을 바꾸지 않고 계산 결과만 돌려주는 함수로, {ch-go}에서 자세히 다룬다)로 분리되어 있다.
...
다음 상태와 복습 시각, 결과 두 개를 함께 돌려주는 시그니처에서 {ch-go-hello}에서 익힌 다중 반환이 실전에 쓰이는 모습도 보인다.
DB 상태와 계산 로직이 1:1로 대응한다는 점이 요지로, 스키마를 설계할 때 "알고리즘이 다음 계산에 쓰는 최소 상태가 무엇인가"를 물으면 컬럼 목록이 저절로 나온다.
...
// max(seq)+1이 Postgres의 identity 열을 대신하며, 로컬은 쓰기 주체가
// 하나뿐이라 경쟁이 없다.
...
셋째, SRS 상태는 알고리즘에 필요한 최소 상태로 컬럼을 구성했고, 복습 기록은 트랜잭션으로 로그 삽입과 상태 갱신을 원자적으로 처리한다.
...
다음 {ch-go-basics}부터는 한 층 올라가, 이 테이블들을 실제로 읽고 쓰는 Go 코드를 읽는다.
