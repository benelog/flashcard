= 17장 html/template으로 만드는 화면

15장에서 만든 API는 JSON 데이터를 돌려줄 뿐이다.
덱 목록을 요청하면 중괄호와 따옴표로 묶인 텍스트가 올 뿐, 사람이 볼 화면은 아니다.
16장에서는 그 화면의 재료인 HTML과 CSS를 실제 파일로 읽었다.
이제 남은 질문은 하나다.
...
3장에서 본 컨텍스트(에이전트에게 건네는 대화 내용)와는 다른 뜻으로, 여기서는 템플릿이 지금 들여다보고 있는 데이터 한 덩어리를 가리킨다.
...
조각을 단독으로 그리는 `renderPartial`도 있는데, 페이지 일부만 갈아 끼우는 htmx 응답용이므로 18장에서 다룬다.
...
15장의 API 핸들러가 "요청을 읽고, 스토어를 다녀와서, JSON으로 응답"이었다면, 페이지 핸들러는 마지막 단계만 "템플릿 렌더링"으로 바뀐다.
...
15장의 JSON 바인딩과 하는 일은 같고, 데이터가 JSON 본문 대신 폼 인코딩으로 온다는 점만 다르다.
...
쿠키라는 저장소 자체와 로그인 유지에 쓰는 방법은 4부의 24장에서 자세히 다룬다.
...
그 정도만으로도 필드 이름 오타나 조각 이름 변경 같은 흔한 실수는 배포 전에 걸러지고, 14장에서 만든 `go test ./...` 한 번에 같이 돈다.
...
21장에서 보겠지만, 이런 반복 지시는 결국 훅이나 스킬로 옮기는 것이 정답이다.
...
페이지 핸들러의 뼈대는 15장의 API 핸들러와 같았다.
