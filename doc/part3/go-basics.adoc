...
가운데의 `if` 분기는 설정에 따라 SQLite 파일로 도는 로컬 모드와 PostgreSQL 모드 중 어느 쪽으로 돌지 정하는데, 그 사정은 {ch-gin}과 {ch-local-dev}에서 다루므로 지금은 문법의 재료로만 쓰겠다.
...
각 모드에서 무엇이 달라지는지는 {ch-gin}과 {ch-local-dev}의 몫이다.
