...
{ch-go-hello}과 {ch-go-types}에서 Go 문법을 파일 하나짜리 예제로 익혔고, {ch-database}에서는 이 앱의 데이터가 머무는 테이블을 해부했다.
이제 한 층 올라와, 그 문법으로 쓴 코드가 실제 저장소에서 어떤 모습인지 볼 차례다.
Flashcard의 백엔드 API는 Go(고)로 작성해 Vercel 서버리스 함수(Serverless Function)로 배포한다.

이 장은 코드를 담는 그릇에서 시작한다.
파일이 열 개, 백 개로 늘면 "이 문법을 어떻게 쓰는가"만큼이나 "이 코드를 어디에 두는가"가 중요해지기 때문이다.
먼저 모듈과 패키지, 그리고 이 저장소의 디렉터리 배치를 확인하고, 이어서 {ch-go-hello}과 {ch-go-types}에서 익힌 변수와 기본 타입, 함수와 다중 반환값, 제어문, 상수가 실제 코드에서 어떤 모습인지 하나씩 짚는다.
이 장의 중심 예제는 환경 변수를 읽어 서버 설정을 만드는 `internal/config/config.go`로, 60여 줄짜리 파일 하나에 그 문법 요소가 거의 다 들어 있다.
여기서 다진 기초는 다음 {ch-go}에서 간격 반복(Spaced Repetition) 알고리즘을 구현한 `internal/srs` 패키지를 비롯한 도메인 코드(암기 학습이라는 이 앱 고유의 문제를 다루는 코드)를 본격적으로 읽기 위한 준비 운동이다.
...
웹 서버 하나에 바깥 의존성이 여덟 개뿐이라는 사실이 {ch-go-hello}에서 말한 표준 라이브러리의 두터움을 숫자로 보여 준다.
...
`cmd/server/main.go`의 핵심 부분으로, 설정을 읽고 그에 맞는 저장소로 앱을 조립해 서버를 띄우는 시작 코드다.
...
가운데의 `if` 분기는 설정에 따라 SQLite 파일로 도는 로컬 모드와 PostgreSQL 모드 중 어느 쪽으로 돌지 정하는데, 그 사정은 {ch-app-assembly}과 {ch-local-dev}에서 다루므로 지금은 문법의 재료로만 쓰겠다.
...
이 조립 과정의 내부는 {ch-app-assembly}에서 다룬다.

== 변수와 기본 타입

코드를 담을 틀을 확인했으니, 이제 그 안을 채우는 문법으로 들어가자.
이 절의 중심 예제는 `internal/config/config.go`다.
...
`config.go`는 환경 변수 몇 개를 읽어 설정 꾸러미 하나를 채우는, 그 일만 하는 파일이다.

파일의 앞부분으로, 설정값 열 개를 한 묶음으로 담을 그릇을 정의한다.
주석에 보이는 Supabase·JWT 관련 값들은 {ch-supabase-auth}에서 다루니, 지금은 칸 이름과 그 뒤의 타입 표기만 보면 된다.
...
구조체 문법은 {ch-go-types}에서 익혔고, 실전 구조체의 해부는 {ch-go}에서 한다.
여기서는 "이름표가 붙은 칸이 열 개 있는 상자"라는 것만 확인하고 넘어가자.
...
슬라이스의 동작 원리는 {ch-go}에서 자세히 본다.

이 파일에는 등장하지 않지만 자주 쓰는 기본 타입이 몇 가지 더 있다.
정수는 `int`, 참/거짓 두 값만 갖는 불리언은 `bool`, 소수점 있는 숫자는 `float64`다.
{ch-go}에서 볼 SRS 상태 구조체가 `float64`와 `int` 필드로 이뤄져 있어 이 타입들을 곧 실물로 만난다.

변수를 만드는 방법은 두 가지다.
`var 이름 타입` 형태의 정식 선언과, 선언과 대입을 한 번에 하는 단축 선언 `:=`다.
`cmd/server/main.go`에서 이미 본 두 줄에 두 형태가 다 있다.
...
`main.go`에서는 `if` 분기의 양쪽에서 대입할 변수를 분기 밖에서 먼저 만들어 두려고 이 형태를 썼다.
두 형태의 중요한 차이는 쓸 수 있는 위치다.
`:=`는 함수 안에서만 쓸 수 있고, 함수 밖(패키지 수준)의 변수는 `var`로 선언해야 한다.
그래서 관례도 자연스럽게 "함수 안에서는 짧은 `:=`, 패키지 수준에서는 `var`"로 굳어져 있다.

{ch-go-hello}에서 본 제로값도 여기 있다.
방금 본 `engine`은 대입되기 전까지 포인터의 제로값인 `nil`이다.
제로값은 {ch-go}에서 "제로값이 곧 유효한 초기 상태가 되도록 구조체를 설계한다"는 원칙으로 다시 등장하니 기억해 두자.

== 함수와 다중 반환값

`config.go`의 나머지 절반은 `Load` 함수 하나다.
...
각 모드에서 무엇이 달라지는지는 {ch-app-assembly}과 {ch-local-dev}의 몫이다.
`*Config`의 `*`는 `Config`가 놓인 곳을 가리키는 포인터(Pointer) 표시인데, 포인터는 {ch-go}에서 제대로 다룬다.
지금은 반환값의 짝에 집중하자.
성공하면 채워진 설정과 `nil`(에러 없음)을, 실패하면 `nil`(설정 없음)과 에러를 돌려준다.

{ch-go-hello}에서 예외 대신 반환값으로 실패를 돌려주는 Go의 관례를 봤는데, `Load`가 그 전형이다.
실패할 수 있는 함수는 마지막 반환값으로 `error` 타입을 함께 돌려주고, 호출한 쪽은 그 값이 `nil`인지부터 확인한다.

호출하는 쪽의 모양은 앞서 본 `cmd/server/main.go`의 첫머리에 있다.
...
{ch-go-hello}에서 본 대로 Go는 이름의 첫 글자로 공개 범위를 정한다.
`Load`와 `Config`가 대문자인 덕분에 다른 패키지에서 `config.Load()`처럼 불러 쓸 수 있고, `main.go`가 부른 `app.New`와 `app.Engine`도 마찬가지다.
...
`config.go`에서 접속 허용 주소 목록을 다듬는 마지막 부분이다.
...
`main.go`의 마지막에 있던 `if err := engine.Run(":" + cfg.Port); err != nil`도 같은 형태로, 함수 호출과 에러 검사를 한 줄로 묶은 관용구다.

=== for: 반복문

Go의 반복문은 `for`다.
횟수를 세는 형태, 조건만 적는 형태, 목록을 훑는 형태는 {ch-go-hello}에서 익혔다.
...
`internal/handlers/sessions.go`에서, 학습 세션을 시작할 때 요청받은 방식(`body.Mode`)에 따라 학습할 카드를 고르는 분기다.
...
이 코드가 놓인 HTTP 핸들러의 전체 구조는 {ch-gin}의 주제이므로, 여기서는 문법만 확인하고 넘어가자.
...
`internal/model/deckslug.go`의 첫 부분을 보자.
...
이 파일의 인코딩이 정말 겹치지 않는 슬러그를 만드는지 확인하는 테스트는 {ch-go-testing}에서 읽는다.

== 에이전트 활용 아이디어

Go의 코드 조직과 기본 문법을 에이전트와 함께 다룰 때 보태 둘 만한 것들이다.

`CLAUDE.md`에는 이 저장소의 디렉터리 규약을 적어 둘 만하다.
...
첫째, Go의 코드 조직은 `go.mod`의 모듈 선언과 `cmd`·`internal`·`pkg` 디렉터리 관례로 이뤄진다.
...
넷째, 제어문은 단출하다.
`if`는 초기화문을 품을 수 있고, 반복은 `for`로 하며 `range`로 목록을 순회하고, `switch`는 `case`마다 자동으로 끝난다.
