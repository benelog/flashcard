...
8장과 9장에서 Go 문법을 파일 하나짜리 예제로 익혔고, 11장에서는 이 앱의 데이터가 머무는 테이블을 해부했다.
...
먼저 모듈과 패키지, 그리고 이 저장소의 디렉터리 배치를 확인하고, 이어서 8장과 9장에서 익힌 변수와 기본 타입, 함수와 다중 반환값, 제어문, 상수가 실제 코드에서 어떤 모습인지 하나씩 짚는다.
이 장의 중심 예제는 환경 변수를 읽어 서버 설정을 만드는 internal/config/config.go로, 60여 줄짜리 파일 하나에 그 문법 요소가 거의 다 들어 있다.
여기서 다진 기초는 다음 13장에서 간격 반복(Spaced Repetition) 알고리즘을 구현한 internal/srs 패키지를 비롯한 도메인 코드(암기 학습이라는 이 앱 고유의 문제를 다루는 코드)를 본격적으로 읽기 위한 준비 운동이다.
...
웹 서버 하나에 바깥 의존성이 여덟 개뿐이라는 사실이 8장에서 말한 표준 라이브러리의 두터움을 숫자로 보여 준다.
...
가운데의 `if` 분기는 설정에 따라 SQLite 파일로 도는 로컬 모드와 PostgreSQL 모드 중 어느 쪽으로 돌지 정하는데, 그 사정은 15장과 19장에서 다루므로 지금은 문법의 재료로만 쓰겠다.
...
주석에 보이는 Supabase·JWT 관련 값들은 24장에서 다루니, 지금은 칸 이름과 그 뒤의 타입 표기만 보면 된다.
...
구조체 문법은 9장에서 익혔고, 실전 구조체의 해부는 13장에서 한다.
...
제로값은 13장에서 "제로값이 곧 유효한 초기 상태가 되도록 구조체를 설계한다"는 원칙으로 다시 등장하니 기억해 두자.
...
각 모드에서 무엇이 달라지는지는 15장과 19장의 몫이다.
`*Config`의 `*`는 `Config`가 놓인 곳을 가리키는 포인터(Pointer) 표시인데, 포인터는 13장에서 제대로 다룬다.
지금은 반환값의 짝에 집중하자.
성공하면 채워진 설정과 `nil`(에러 없음)을, 실패하면 `nil`(설정 없음)과 에러를 돌려준다.

8장에서 예외 대신 반환값으로 실패를 돌려주는 Go의 관례를 봤는데, `Load`가 그 전형이다.
...
8장에서 본 대로 Go는 이름의 첫 글자로 공개 범위를 정한다.
...
이 코드가 놓인 HTTP 핸들러의 전체 구조는 15장의 주제이므로, 여기서는 문법만 확인하고 넘어가자.
...
이 파일의 인코딩이 정말 겹치지 않는 슬러그를 만드는지 확인하는 테스트는 14장에서 읽는다.
