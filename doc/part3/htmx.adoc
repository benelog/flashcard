= 18장 htmx: 자바스크립트 없이 만드는 동적 화면

16장에서 화면의 구조와 모양을 만드는 HTML·CSS를, 17장에서 서버가 데이터를 채워 HTML을 완성해 내려보내는 html/template을 살펴봤다.
...
17장에서 만든 서버 렌더링 구조가 그대로 연장되므로, 화면을 그리는 로직은 여전히 Go 한 곳에만 있다.
...
브라우저와 서비스 워커가 낡은 사본을 새 화면에 물리지 않게 하려는 장치이고, 29장에서 다룬다.
지금은 세 파일을 불러오는 줄이라고만 읽으면 된다.

htmx.min.js는 50KB 남짓한 파일 하나다.
남이 만든 파일을 외부 서버에서 내려받는 대신 저장소에 직접 복사해 두는 방식(벤더링, vendoring)을 택했고, 17장에서 본 embed로 Go 바이너리에 함께 넣었다.
...
`closest li`는 16장에서 본 가족 관계를 그대로 쓴 표현이다.
...
17장에서 본 render는 layout 템플릿부터 실행해 `<html>`로 시작하는 페이지 전체를 만들었다.
...
partials가 templates/partials/ 아래 파일들로 만들어진다는 것은 17장의 템플릿 파싱 코드에서 봤다.
...
3장의 대화 세션(에이전트와 나누는 한 번의 대화)이나 24장에서 볼 로그인 세션과는 다른 뜻이고, 곧 나올 "세션 저장 공간"의 세션은 서버가 사용자마다 마련해 두는 기억 자리를 가리킨다.

이 상태를 어디에 둘 것인가.

첫째, 서버 메모리에 둘 수 있다.
전통적인 서버라면 사용자별 세션 저장 공간에 진행 상태를 담아 두는 것이 자연스럽다.
그러나 이 앱의 운영 환경인 Vercel 서버리스 함수(23장)는 요청이 뜸하면 잠들었다가 새로 깨어나고(콜드스타트), 그때 메모리는 백지가 된다.
...
HTTP는 본래 요청 하나하나가 독립인 무상태 프로토콜인데, 이 설계는 그 성질을 거스르지 않고 그대로 올라탄다(무상태라는 개념은 24장에서 정리한다).
...
흐름을 따라가 보자.
stateFromForm이 폼 값에서 studyState를 다시 조립하고, 큐의 첫 카드를 꺼내 채점 결과를 데이터베이스에 기록한다(이것이 서버에 남는 유일한 것으로, 11장에서 설계한 리뷰 기록이다).
...
외부 API 호출과 응답 가공이 Go 코드에 있으므로 브라우저 자바스크립트가 필요 없고, 응답을 두 줄 요약으로 다듬는 로직(summarizeEntries)은 14장에서 본 방식대로 Go 테스트로 검증할 수 있다.
...
서버가 평소처럼 303 리다이렉트(17장의 PRG 패턴에서 썼던, 브라우저를 다른 주소로 이동시키는 응답)를 보내면 곤란한 일이 생긴다.
...
카드 뒤집기조차 16장에서 본 대로 숨긴 체크박스와 CSS만으로 동작한다.
...
브라우저가 자기 시간대 이름을 쿠키에 적어 두면 이후 모든 요청에 실려 가고, 서버의 clientTZ 함수가 그것을 읽어 날짜 경계를 계산한다(쿠키의 동작 원리는 24장에서 다룬다).
둘째 덩어리는 서비스 워커 등록으로, 앱을 홈 화면에 설치하고 오프라인에서도 열리게 만드는 준비 작업이다.
자세한 내용은 29장 PWA에서 다루므로 여기서는 한 줄 등록만 봐 두자.
...
이 장으로 16장에서 시작한 화면 이야기 세 편이 마무리됐다.
16장에서 문서(HTML)와 스타일(CSS)을, 17장에서 서버가 데이터를 채우는 html/template을, 이번 장에서 새로고침 없는 동적 화면을 만드는 htmx를 살펴봤다.
...
다음 19장에서는 5장에서 명령 하나로 띄웠던 이 앱이 어떻게 설정 없이 돌 수 있었는지 그 원리를 해부하고, 코드를 고치는 사람 쪽 개발 환경까지 갖춰 3부를 마무리한다.
