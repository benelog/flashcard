...
서버 쪽 핸들러를 보자.
다음은 internal/web/cards.go의 deleteCard로, 같은 주소가 htmx 요청과 일반 폼 제출을 모두 처리한다.
...
다음은 internal/web/render.go의 renderPartial로, 조각 전용 템플릿 묶음(Web 타입의 partials 필드)에서 이름 붙은 템플릿 하나만 실행해 그대로 써 보낸다.
...
다음은 internal/web/decks.go의 deleteDeck으로, 요청이 어디서 왔는지에 따라 두 가지 이동 방식을 골라 쓴다.
