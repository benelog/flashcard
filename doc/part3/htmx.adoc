...
서버 쪽 핸들러를 보자.
다음은 `internal/web/cards.go`의 deleteCard로, 같은 주소가 htmx 요청과 일반 폼 제출을 모두 처리한다.
...
{ch-go-templates}에서 본 render는 layout 템플릿부터 실행해 `<html>`로 시작하는 페이지 전체를 만들었다.
...
다음은 `internal/web/render.go`의 renderPartial로, 조각 전용 템플릿 묶음(Web 타입의 partials 필드)에서 이름 붙은 템플릿 하나만 실행해 그대로 써 보낸다.
...
partials가 templates/partials/ 아래 파일들로 만들어진다는 것은 {ch-go-templates}의 템플릿 파싱 코드에서 봤다.

여기서 중요한 설계가 하나 있다.
페이지가 처음 로딩될 때의 화면과, htmx가 갈아 끼울 조각이 같은 템플릿 정의를 공유한다는 것이다.
다음은 `internal/web/templates/pages/study.html`로, 학습 페이지의 내용이 study_body라는 조각 템플릿을 불러 그리는 것이 전부다.
...
이 구조체가 HTML로 변신한 모습이 `internal/web/templates/partials/study.html`의 study_state_fields다.
...
이제 서버가 이 폼을 받는 쪽이다.
다음은 `internal/web/study.go`의 gradeCard로, 채점 한 번에 벌어지는 모든 일이다.
...
흐름을 따라가 보자.
stateFromForm이 폼 값에서 studyState를 다시 조립하고, 큐의 첫 카드를 꺼내 채점 결과를 데이터베이스에 기록한다(이것이 서버에 남는 유일한 것으로, {ch-database}에서 설계한 리뷰 기록이다).
...
다음은 `internal/web/decks.go`의 deleteDeck으로, 요청이 어디서 왔는지에 따라 두 가지 이동 방식을 골라 쓴다.
...
// ... 로그아웃 뒤 서비스 워커의 페이지 캐시를 비우는 덩어리는 34장에서 본다 ...
