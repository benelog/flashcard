...
브라우저와 서비스 워커가 낡은 사본을 새 화면에 물리지 않게 하려는 장치이고, {ch-pwa}에서 다룬다.
...
`closest li`는 {ch-html-css}에서 본 가족 관계를 그대로 쓴 표현이다.
...
// ... 로그아웃 뒤 서비스 워커의 페이지 캐시를 비우는 덩어리는 29장에서 본다 ...
...
자세한 내용은 {ch-pwa} PWA에서 다루므로 여기서는 한 줄 등록만 봐 두자.
...
다음 {ch-local-dev}에서는 {ch-first-run}에서 명령 하나로 띄웠던 이 앱이 어떻게 설정 없이 돌 수 있었는지 그 원리를 해부하고, 코드를 고치는 사람 쪽 개발 환경까지 갖춰 3부를 마무리한다.
