...
간격 반복(Spaced Repetition) 상태를 저장하는 데이터 모델과 URL slug(주소에 쓰는 짧은 식별 문자열) 설계까지가 이 장의 범위이고, 같은 설계가 운영 PostgreSQL에서 어떤 전용 타입과 장치로 표현되는지는 22장에서 다룬다.
...
인증 시스템의 식별자와 앱 데이터의 식별자를 일치시키면 조인 없이 인증된 사용자 ID로 바로 조회할 수 있다(운영의 인증 흐름은 21장에서 다룬다).
...
"오답률 0.4 이상" 같은 규칙 조건과 store 계층(DB를 읽고 쓰는 Go 코드로, 9장과 11장에서 다룬다)의 카드 조회가 모두 이 뷰 하나를 바라보므로, 오답률 계산식이 한 곳에만 존재한다.
...
운영 스키마도 같은 전략인데, 그쪽은 uuid 타입에 `gen_random_uuid()` 기본값, 순번에는 identity 컬럼이라는 전용 장치가 있다는 점만 다르다(22장).
...
상태 전이 로직은 `internal/srs/srs.go`에 순수 함수(입력을 바꾸지 않고 계산 결과만 돌려주는 함수로, 9장에서 자세히 다룬다)로 분리되어 있다.
...
"덱마다 순번 하나"라는 같은 설계가 환경의 동시성 수준에 따라 다른 채번 장치로 구현되는 대비로, 운영 쪽 장치는 22장에서 본다.
...
고치는 절차와 지키는 장치, 이 두 이야기는 운영 스키마(`internal/db/migrations/`)와 함께 22장에서 다룬다.
...
이 운영 이야기는 PostgreSQL의 전용 타입들과 함께 22장에서 이어진다.

여기까지가 데이터가 머무는 바닥층이다.
다음 8장부터는 한 층 올라가, 이 테이블들을 실제로 읽고 쓰는 백엔드 언어 Go의 기초를 익힌다.
