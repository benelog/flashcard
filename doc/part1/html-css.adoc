= 14장 HTML과 CSS: 화면을 이루는 문서와 스타일

12장과 13장에서 HTML과 CSS의 문법을 파일 하나짜리 예제로 익혔다.
...
15장에서는 Go 서버가 html/template으로 HTML을 데이터에 맞춰 찍어 내는 방법을, 16장에서는 htmx로 화면 일부만 갈아 끼우는 방법을 다룬다.
...
`{{define "layout"}}`이나 `{{.Title}}`처럼 중괄호 두 겹으로 감싼 부분은 15장에서 다룰 서버 템플릿 문법이므로, 이 장에서는 "서버가 값을 채워 넣는 자리" 정도로만 읽고 넘어가면 된다.
...
`<title>`은 브라우저 탭에 표시되는 제목이고, `<link rel="stylesheet">`는 이 장 후반의 주인공인 CSS 파일을, `<link rel="manifest">`는 23장 PWA에서 다룰 웹 앱 매니페스트를 불러온다.
`<script>` 두 줄은 htmx 라이브러리(16장)와 이 앱의 유일한 자바스크립트 파일을 불러오고, `defer`는 "문서를 다 읽은 뒤에 실행하라"는 뜻이라 스크립트가 화면 그리기를 막지 않는다.
...
12장에서 본 시맨틱 마크업이 이 파일에 그대로 적용되어 있다.
...
자바스크립트로 화면 전환을 직접 구현하는 SPA 방식과의 비교는 1장에서 다뤘고, 이 방식의 빈틈(전체 새로고침이 어색한 장면)을 메우는 htmx는 16장에서 만난다.

== 폼: 화면에서 서버로 데이터를 보낸다

지금까지는 서버가 브라우저로 보내는 방향이었다.
반대 방향, 즉 사용자가 입력한 것을 서버로 보내는 표준 도구가 12장에서 본 폼이다.
...
`method="post"`는 11장에서 본 POST, 즉 "새로 만들어 달라"는 요청이라, "만들기" 버튼을 누르면 브라우저가 입력 값을 모아 `POST /decks` 요청을 보내고 서버가 덱을 만든 뒤 새 화면으로 안내한다.
...
서버가 이 파일을 받아 카드로 바꾸는 Go 코드(encoding/csv)는 15장에서 다룬다.
...
Flashcard의 화면은 될 수 있으면 상태를 HTML 요소에 들려 두고 CSS가 그것을 읽어 겉모습만 바꾸며, 16장의 학습 화면은 진행 상태를 폼에 실어 서버와 왕복시킨다.
====
...
화면이 열두어 개인 1인 프로젝트 Flashcard는 빌드 없이 파일 하나로 끝나는 순수 CSS를 골랐고, 이 선택의 전체 그림은 1장에서 정리했다.
...
13장에서 본 대로 현대 CSS의 배치 도구는 플렉스박스와 그리드 둘이다.
...
채점 폼에 붙은 `hx-`로 시작하는 속성들은 16장에서 다룰 htmx의 문법이니 지금은 접어 두자.
...
카드를 뒤집은 다음 단계, 즉 채점 결과를 서버에 보내고 다음 카드를 받아 오는 일은 CSS만으로는 안 되므로 16장의 htmx가 이어받는다.
