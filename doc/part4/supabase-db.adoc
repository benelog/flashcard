= 26장 Supabase 데이터베이스: pgx 연결과 커넥션 풀

24장과 25장에서 인증을 마쳤으니, 검증을 통과한 요청은 이제 데이터베이스로 내려간다.

그런데 이 앱에는 이미 데이터베이스가 있다.
3부까지 앱은 환경 변수 하나 없이 `go run ./cmd/server`로 떠서, SQLite 파일 하나에 덱과 학습 기록을 쌓는 것으로 완성됐다(5장).
이 장에서는 그 앱을 운영 데이터베이스, 즉 Supabase의 관리형 PostgreSQL로 옮긴다.

먼저 이 전환이 코드 수정이 아니라 환경 변수 하나로 끝나는 구조를 확인하고, 11장에서 설계한 스키마가 PostgreSQL에서 얻는 더 풍부한 타입들을 살펴본다.
...
답은 15장에서 본 handlers→store 계층 분리에 있다.
...
11장에서 한 설계(일곱 테이블, 외래 키와 cascade, 조회 패턴에서 역산한 인덱스)는 PostgreSQL에서도 그대로고, 바뀌는 것은 표현이다.
SQLite는 타입의 가짓수를 최소로 줄인 대신 어디서나 도는 쪽을 택한 DB라 11장의 스키마는 많은 것을 `text` 열에 눌러 담았는데, PostgreSQL은 그 자리마다 전용 타입을 내준다.

아래 표의 왼쪽은 11장에서 SQLite가 전용 타입이 없어 문자열에 눌러 담았던 자리이고, 오른쪽은 PostgreSQL이 그 자리마다 내주는 전용 타입이다.
오른쪽 이름들이 낯설어도 지금 외울 필요는 없고, 표 아래에서 하나씩 풀어 나간다.

|===
| 11장의 SQLite 표현 | 운영 PostgreSQL
...
사용자와 카드 테이블을 PostgreSQL 타입으로 만드는 코드로, 11장의 SQLite 스키마와 나란히 놓고 읽으면 대응이 바로 보인다.
...
카드의 두 열이 `text`·`meaning`이 아니라 `front_text`·`back_text`라는 첫 이름으로 등장하는데, 이 이름이 왜 바뀌었는지는 27장의 마이그레이션에서 확인한다.

`settings`의 `jsonb`는 JSON을 문자열이 아니라 파싱된 이진 형태로 저장하는 타입으로, 넣을 때 문법이 검증되고 내부 필드 조회와 인덱스도 가능하다.
`card_type`은 11장에서 `check` 제약으로 흉내 냈던 값 목록이 enum 타입이 된 모습으로, 타입 자체가 문서가 되고 저장 공간도 작지만, 값 목록을 바꾸는 일이 데이터 수정이 아니라 틀을 고치는 DDL이 되고 특히 값 제거는 불가능에 가깝다는 대가가 있다.
이 비용도 27장에서 실제 사례로 확인한다.

=== 기본 키: 데이터베이스가 UUID를 만든다

`profiles.id`에는 `default`가 없다.
이 값은 DB가 만들지 않고 Supabase Auth가 발급한 사용자 UUID를 그대로 받아 쓰기 때문이다.
25장에서 본 JWT의 사용자 식별자(`sub` 클레임)가 바로 이 값이라, 인증의 식별자와 앱 데이터의 식별자가 일치해 조인 없이 토큰의 사용자 ID로 바로 조회할 수 있다.
...
그래서 빠르게 쌓이고 외부에 노출되지 않는 `review_logs`에는 순번 키(`bigint generated always as identity`)를 써서 "노출되는 것은 UUID, 내부 대량 로그는 순번"이라는 이원 전략을 취했고, 11장의 SQLite에서는 `integer primary key autoincrement`가 그 자리를 맡았다.
...
11장의 SQLite에는 시간 타입 자체가 없어 고정 폭 UTC 문자열로 저장하고 문자열 비교가 곧 시간 비교가 되게 맞췄는데, 그 수공업이 여기서는 타입 하나로 끝난다.
...
10장에서 본 B-tree 인덱스는 한 칸에 값 하나가 든 경우를 정렬해 찾는 구조라 배열 안의 개별 원소는 훑지 못하므로, 배열 컬럼에는 GIN(Generalized Inverted Index) 인덱스를 건다.
...
`go.mod`의 DB 관련 의존성은 세 개로, PostgreSQL 드라이버 `github.com/jackc/pgx/v5`, SQLite 드라이버 `modernc.org/sqlite`, 그리고 27장에서 볼 마이그레이션 도구 `github.com/golang-migrate/migrate/v4`다.
...
여기서 용어 하나를 못 박아 두자.
이 절에서 말하는 세션은 DB에 접속해 있는 연결 하나를 뜻하며, 3장의 대화 세션과 24장의 로그인 세션, 그리고 이 앱의 학습 세션(`study_sessions` 테이블)과는 모두 다른 뜻이다.
...
세 가지 결정이 담겨 있다.
첫째, `sync.Once`로 풀을 프로세스당 하나만 만들어 웜 인스턴스가 호출 간에 재사용한다(25장에서 본 JWKS 클라이언트와 같은 패턴이다).
...
단순 프로토콜에서 파라미터는 pgx가 안전하게 이스케이프한 텍스트로 쿼리에 인코딩되므로 SQL 인젝션(10장) 걱정은 없다.
...
23장에서 본 `vercel.json`은 함수 리전을 고정하고 있다.
...
이때 접속자는 `anon`(로그인 전)과 `authenticated`(로그인 후)라는 두 역할 중 하나로 취급되고, 문을 여는 열쇠는 24장에서 본 anon key다.
...
세션 저장소를 두지 않은 24장의 무상태 인증과 짝을 이루는 결정이다.
...
30장에서 보겠지만 무료 플랜에는 자동 백업조차 없어서, 운영 데이터 위의 실수는 돌아갈 시점이 없는 사고가 된다.

특히 27장에서 다룰 마이그레이션이 그렇다.
...
일시정지된 프로젝트는 이 수에 들어가지 않는데, 이 한도가 무료 티어 살림에 갖는 의미는 30장에서 다시 짚는다.

개발 환경을 아예 다른 플랫폼에 차리는 안도 검토했다.
그런데 23장에서 봤듯 2026년 7월 기준으로 카드 등록 없이 무료로 서버를 올릴 수 있는 플랫폼 자체가 드물고, 유일하게 가능한 Render도 무료 PostgreSQL이 생성 30일 뒤 만료되어 계속 유지해야 하는 개발 DB에는 맞지 않았다.
...
이 두 프로젝트는 20장에서 정한 브랜치 정책, 23장에서 설정한 배포 구분과 나란히 개발·운영으로 정렬된다.

|===
| 층 | 코드 | 배포 | 데이터베이스

| 로컬 | 작업 중인 코드 | 없음 | SQLite 파일(5장)
| 개발 | `main` 브랜치 | Vercel 프리뷰 배포 | 개발 Supabase 프로젝트
| 운영 | `release` 브랜치 | Vercel 프로덕션 배포 | 운영 Supabase 프로젝트
|===

배선은 23장에서 본 환경 변수 스코프가 담당한다.
...
스키마를 이 배선 위에서 누가 언제 적용하는지, 곧 마이그레이션의 체계는 27장에서 세운다.
로컬에서 개발 DB에 실제로 붙는 절차와 두 벌이 된 값을 관리하는 요령은 28장에서 다룬다.
...
스키마를 실제로 고치는 작업의 지침은 27장에서 마이그레이션 체계와 함께 적는다.
...
둘째, 설계는 같고 타입이 풍부해진다.
11장에서 `text`로 눌러 담았던 자리에 uuid·timestamptz·jsonb·배열·enum·identity가 들어서고, 태그 검색은 GIN 인덱스가 받친다.
...
24장에서 본 무상태 인증에 이 결정이 더해져, 검증할 표면적이 API 한 곳으로 줄어든다.
...
다음 27장에서는 이 데이터베이스의 틀을 운영 중에 고치는 문제를 다룬다.
