= 24장 Supabase 인증: OAuth 로그인과 쿠키 세션

23장에서 Go 서버를 Vercel 서버리스 함수로 배포하는 과정을 살펴봤다.
남은 조각은 사용자 인증과 데이터베이스이고, 둘 다 Supabase가 맡는다.
성격이 꽤 다른 두 이야기라 장을 나눴다.
이 장과 다음 25장은 인증을, 26장과 27장은 데이터베이스를 다룬다.
...
여정 끝에 쿠키로 돌아온 토큰이 진짜인지 서버가 확인하는 원리는 다음 25장에서 해부한다.
로그인을 따라 만드는 데에는 이 장까지로 충분하다.

무료 플랜의 한도와 일시정지·백업 같은 운영 문제는 30장에서 Vercel과 함께 묶어 다룬다.
...
다만 무료에는 무료의 규칙이 따라붙는데, 그 한도와 함정은 장 첫머리에서 말한 대로 30장의 몫이다.
...
이 장에서 계속 만날 인가 코드·인가 화면·인가 주소의 "인가"는 사용자가 프로바이더 화면에서 우리 앱의 로그인 요청을 허가하는 OAuth 절차를 가리키는 말로, 10장에서 본 인가(그 사람이 이 일을 해도 되는지 앱 안에서 확인하는 권한 검사)와는 다른 뜻이다.
...
3장의 토큰(LLM이 글을 쪼개 세는 단위)과는 이름만 같을 뿐 다른 말이고, 이 장의 토큰은 로그인 상태를 증명하는 문자열이다.
====
...
그러나 Firestore는 NoSQL 문서 저장소라서 11장에서 설계한 관계형 스키마(외래 키 제약, JOIN, 집계 쿼리)를 그대로 옮길 수 없다.
...
PostgREST를 쓰지 않는 이유는 데이터 접근 규칙이 단순 CRUD(23장)가 아니기 때문이다.
...
그러나 Flashcard의 화면은 17장과 18장에서 본 대로 서버에서 렌더링되고, 곧 확인하겠지만 로그인 흐름에도 브라우저 자바스크립트가 필요 없다.
...
이 책에서는 쿠키로 유지되는 로그인 상태를 로그인 세션이라 부르는데, 3장에서 말한 세션(Claude Code와 나누는 대화 한 판)과는 전혀 다른 뜻이다.
====
...
악성 스크립트를 우리 페이지 안에 심는 XSS(17장)와는 다른 공격이다.
...
토큰이 유효한지 가리는 `auth.ParseUser`의 속(서명과 클레임 검증)은 25장에서 해부한다.
...
데이터에 닿는 길은 Go 서버의 직접 연결(26장) 하나뿐이다.
...
19장의 로컬 모드는 로그인 자체가 없으므로 이 등록과 무관하다.
환경 변수를 채워 운영 구성 그대로 내 컴퓨터에서 띄웠을 때, 실제 Google/GitHub 로그인 왕복을 배포 전에 확인하기 위한 주소다.

== 에이전트 활용 아이디어

인증은 3장의 검토 기준표에서 오른쪽 아래, 즉 오류 발견이 어렵고 비용이 큰 영역이다.
...
다음 25장에서는 이 장이 쿠키에 담아 둔 토큰의 속을 연다.
