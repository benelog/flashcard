...
저장소에 Go 모듈이 셋 있다: `flashcard-advanced/`(배포용 앱, `github.com/benelog/flashcard`),
`flashcard-basic/`(책 2부의 최소 앱), `flashcard-cli/`(완성본 JSON API를 부르는 CLI). 아래 검사를 순서대로 실행하라:

1. 저장소 루트에서 `gofmt -l .` — 출력이 있으면 포맷 위반
2. `go build ./...` — 세 모듈 디렉터리 각각에서
3. `go vet ./...` — 세 모듈 디렉터리 각각에서
4. `go test ./...` — 세 모듈 디렉터리 각각에서
