...
// deckSource는 내보내기가 저장소에서 쓰는 전부다. 인터페이스는 쓰는 쪽에서
// 최소로 정의한다: model.Store가 이 계약을 포함하므로 호출자는 저장소를 그대로
// 넘기면 된다.
type deckSource interface {
	GetDeckBySlug(ctx context.Context, userID uuid.UUID, slug string) (model.Deck, error)
...
func ExportDeck(ctx context.Context, s deckSource, userID uuid.UUID, slug string, w http.ResponseWriter) error {
